...
\begin{frame}
    ...
    \frametitle{Birth Order}
        {\resizebox{0.9\textwidth}{!}{ \includegraphics{./FIGURES/TWFE/covid_twfe_summ_bysibs_all_all_gpa_m_adj_Tsiblings_Soldest_4.pdf}
      }
    }  
...
\begin{frame}
    ...
    \only<2->{
    \highlightboxflex{0.3}{-0.9}{0.8}{0.7}{draw=blue, draw opacity=0.8, dashed, fill=yellow!30, fill opacity=0.3}
    
    % Highlight box for Covid same sex coefficient  
    \highlightboxflex{3.55}{-0.9}{0.8}{0.7}{draw=blue, draw opacity=0.8, dashed, fill=yellow!30, fill opacity=0.3}
    ...
    \begin{tikzpicture}[overlay, remember picture]
        ...
        \draw[->, thick, blue, opacity=0.4] ([shift={(0,-2.2)}]current page.center) -- ([shift={(0.3,-0.9)}]current page.center);
        
        % Arrow to Covid coefficient (pointing to the right box)
        \draw[->, thick, blue, opacity=0.4] ([shift={(0,-2.2)}]current page.center) -- ([shift={(3.55,-0.9)}]current page.center);
    \end{tikzpicture}
    }
\end{frame}
