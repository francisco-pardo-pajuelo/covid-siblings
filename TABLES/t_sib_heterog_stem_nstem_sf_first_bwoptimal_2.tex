\begin{table}[!htbp]\centering\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}
\caption{Effect on younger sibling choices over STEM majors: Focal child applied to non-STEM and younger sibling is female}\label{tab:t_sib_heterog_stem_nstem_sf_first_bwoptimal_2}
\scalebox{0.7}{\setlength{\textwidth}{.1cm}
\newcommand{\contents}{
\begin{tabular}{lcccccccc}
\toprule
&  \multicolumn{2}{c}{School}          &  \multicolumn{3}{c}{Applies}                          &       \multicolumn{3}{c}{Enrolls}             \\
&  \multicolumn{2}{c}{Progression} &  \multicolumn{3}{c}{ever}                                 &       \multicolumn{3}{c}{ever}                \\
\cline{2-3} \cline{4-6} \cline{7-9} 
&  \multirow{2}{*}{Dropout}    & Secondary             &  Any                  & STEM                          & Non-STEM      &  Any          & STEM                  & Non-STEM              \\
&                                                      & Complete                      &  Major                & Major                         & Major                 &  Major        & Major                 & Major                 \\
& (1)                                                  & (2)                           & (3)                   & (4)                           & (5)                   & (6)           & (7)                   & (9)                   \\
\bottomrule
&  &  &  & & & & & \\
\multicolumn{9}{l}{Panel A: Focal child is a female (sex-match)} \\
Older sibling enrolled&       0.008   &       0.038   &       0.058   &      -0.027   &       0.089   &      -0.042   &      -0.045   &       0.015   \\
                    &     (0.013)   &     (0.045)   &     (0.058)   &     (0.049)   &     (0.064)   &     (0.068)   &     (0.036)   &     (0.067)   \\
 
Older sibling above cutoff&       0.004   &       0.018   &       0.027   &      -0.012   &       0.041   &      -0.019   &      -0.020   &       0.007   \\
                    &     (0.006)   &     (0.021)   &     (0.027)   &     (0.022)   &     (0.030)   &     (0.032)   &     (0.017)   &     (0.031)   \\
                    &               &               &               &               &               &               &               &               \\
Observations        &      24,302   &       5,131   &       5,733   &       6,607   &       5,683   &       5,441   &       7,117   &       5,589   \\
Counterfactual mean &       0.043   &       0.883   &       0.722   &       0.171   &       0.636   &       0.473   &       0.090   &       0.388   \\
Bandwidth           &       0.814   &       0.680   &       0.761   &       0.853   &       0.756   &       0.726   &       0.909   &       0.746   \\
\textit{F}-statistic&       1,248   &         294   &         321   &         354   &         317   &         308   &         389   &         317   \\
 
&  &  &  & & & & & \\
\multicolumn{10}{l}{Panel B: Focal child is a male} \\
Older sibling enrolled&      -0.006   &       0.006   &       0.145*  &      -0.027   &       0.102   &       0.137   &       0.047   &       0.121   \\
                    &     (0.018)   &     (0.063)   &     (0.079)   &     (0.056)   &     (0.084)   &     (0.084)   &     (0.042)   &     (0.083)   \\
 
Older sibling above cutoff&      -0.003   &       0.003   &       0.072*  &      -0.013   &       0.051   &       0.067   &       0.022   &       0.060   \\
                    &     (0.009)   &     (0.031)   &     (0.039)   &     (0.027)   &     (0.041)   &     (0.041)   &     (0.020)   &     (0.041)   \\
                    &               &               &               &               &               &               &               &               \\
Observations        &      14,990   &       2,925   &       3,237   &       3,737   &       3,206   &       3,055   &       4,029   &       3,144   \\
Counterfactual mean &       0.053   &       0.848   &       0.660   &       0.134   &       0.595   &       0.413   &       0.071   &       0.351   \\
Bandwidth           &       0.814   &       0.680   &       0.761   &       0.853   &       0.756   &       0.726   &       0.909   &       0.746   \\
\textit{F}-statistic&         884   &         175   &         199   &         228   &         197   &         187   &         249   &         196   \\
 

\bottomrule \multicolumn{9}{p{\textwidth}} {\footnotesize \textit{Notes:} ***p$<$0.01, **p$<$0.05, *p$<$0.1. }\end{tabular}}\setbox0=\hbox{\contents}
\setlength{\textwidth}{\wd0-2\tabcolsep-.25em} \contents} \end{table}