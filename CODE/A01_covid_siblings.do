...
program define clean_data
	...
	keep id_per_umc year region_siagie public_siagie urban_siagie id_ie level grade male_siagie lives_with_mother lives_with_father approved approved_first std_gpa_m std_gpa_c std_gpa_m_adj std_gpa_c_adj math comm
...
program define raw_gpa_trends
	...
	*- But secondary changed to letters starting with cohort 2013 (7th grade in 2019)
	...
	use std_gpa_?* pass_math pass_read id_per_umc year grade treated fam_order_${fam_type} fam_total_${fam_type} ${x} using "$TEMP\pre_reg_covid${covid_data}", clear
	...
	*- Collapse data
	preserve 	
		collapse std_gpa_m std_gpa_c std_gpa_m_adj std_gpa_c_adj pass_math pass_read, by(year treated)
		...
		foreach v in "std_gpa_m" "std_gpa_c" "std_gpa_m_adj" "std_gpa_c_adj" "pass_math" "pass_read" {
			...
			twoway 		(rarea miny maxy areax if inrange(areax, 2019.5, 2021.5), color(gs15)) ///
						...
						capture qui graph export "$FIGURES\Descriptive\raw_total_`v'${covid_data}.png", replace			
						capture qui graph export "$FIGURES\Descriptive\raw_total_`v'${covid_data}.pdf", replace	
	...
	restore
		...
		collapse std_gpa_m std_gpa_c std_gpa_m_adj std_gpa_c_adj pass_math pass_read, by(year grade treated)
		...
		*- All individual plots	
		...
		graph combine   `v'_1 	///
			...
			///graph save "$FIGURES\raw_m.gph" , replace	
			///capture qui graph export "$FIGURES\raw_m.eps", replace	
			capture qui graph export "$FIGURES\Descriptive\raw_grades_`v'${covid_data}.png", replace			
			capture qui graph export "$FIGURES\Descriptive\raw_grades_`v'${covid_data}.pdf", replace	
		}
