		...
		foreach only_covid in "20-21" "all" {
			...
			estimates clear
				...
				*- All TWFE by # of siblings	
				...
				if "${covid_data}" == "_TEST" {
					capture qui graph export "$FIGURES_TEMP\TWFE\covid_twfe_sibs_summ_`level'_`only_covid'_`vlab'_`type'_${max_sibs}${covid_data}.png", replace	
					capture qui graph export "$FIGURES_TEMP\TWFE\covid_twfe_sibs_summ_`level'_`only_covid'_`vlab'_`type'_${max_sibs}${covid_data}.pdf", replace	
					}
				if "${covid_data}" == "" {
					capture qui graph export "$FIGURES\TWFE\covid_twfe_sibs_summ_`level'_`only_covid'_`vlab'_`type'_${max_sibs}${covid_data}.png", replace	
					capture qui graph export "$FIGURES\TWFE\covid_twfe_sibs_summ_`level'_`only_covid'_`vlab'_`type'_${max_sibs}${covid_data}.pdf", replace	
					}				
			}
		}
	}
					...
					foreach hed in "all" /*"hed" "nhed"*/  { //none or at least one. # No change
						foreach res in "all" /*"alls"*/ /*"nint"*/ /*"ncom" "lses" "nqui"*/ { //all sample with data, No internet, no computer, low ses, no quiet room
							...
							*/
							...
							capture qui graph export "$FIGURES\Event Study\covid_`v'_`area'_`lives_both_parents'_`hed'_`level'`young'_`only_covid'_`res'`i'${covid_data}.png", replace	
							capture qui graph export "$FIGURES\Event Study\covid_`v'_`area'_`lives_both_parents'_`hed'_`level'`young'_`only_covid'_`res'`i'${covid_data}.pdf", replace								
...
foreach g_pair in "g2_6" "g4_6" "g4_7" "g8_9" /*"g8_u"*/ {
...
if "`g_pair'" == "g8_9" {
	...
	*- Match Baseline ECE exams
	...
	rename _m merge_ece_baseline_`g_ece'
...
replace g_pair = 4 if g_pair==.
	...
	reghdfe std_gpa_m_adj treated_post treated post base_score_math base_score_com, a(id_ie g_pair)
	...
	reghdfe std_gpa_m_adj treated_post treated post base_score_math base_score_com if radio==0, a(id_ie)
	reghdfe std_gpa_m_adj treated_post treated post base_score_math base_score_com if radio==1, a(id_ie)
	reghdfe std_gpa_m_adj treated_post treated post base_score_math base_score_com if internet==0, a(id_ie)
	reghdfe std_gpa_m_adj treated_post treated post base_score_math base_score_com if internet==1, a(id_ie)
	reghdfe std_gpa_m_adj treated_post treated post base_score_math base_score_com if pc==0, a(id_ie)
	reghdfe std_gpa_m_adj treated_post treated post base_score_math base_score_com if pc==1, a(id_ie)
	reghdfe std_gpa_m_adj treated_post treated post base_score_math base_score_com if laptop==0, a(id_ie)
	reghdfe std_gpa_m_adj treated_post treated post base_score_math base_score_com if laptop==1, a(id_ie)
