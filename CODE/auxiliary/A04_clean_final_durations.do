...
program define student_final 
	...
	foreach y in "2014" "2015" "2016" "2017" "2018" "2019" "2020" "2021" "2022" "2023" {
		append using "$TEMP\siagie_`y'", keep(id_ie id_per_umc level grade year male_siagie region_siagie public_siagie urban_siagie carac_siagie approved approved_first seccion_siagie comm_secondary math_secondary std_gpa_m std_gpa_c lat_ie lon_ie)
	}
	...
	*- Dropout
	...
	isvar /*Relevant vars*/ id_ie id_per_umc id_ie_??? exp_entry_year exp_graduating_year1 exp_graduating_year2 male_siagie region_siagie public_siagie urban_siagie carac_siagie grade section_siagie year dropout* approved_???? approved_first_???? math_secondary_???? comm_secondary_???? std_gpa_m_???? std_gpa_c_???? change_ie_???? pri_grad sec_grad year_grad_school educ_caretaker educ_mother educ_father id_fam_* fam_order_* fam_total_* lat_ie lon_ie
...
//		bys id_ie: egen avg_applied_private_if 	= mean(cond(exp_graduating_year2>=2016 & exp_graduating_year2<=2022 & applied==1,applied_private,.))
	...
	*- Likelihood of enrollment
	likelihood_predicted_uni
	merge 1:1 id_per_umc using "$TEMP\likelihood_predicted_uni", keepusing(*lpred?) keep(master match) nogen
...
*- Heterogeneity analysis: Likelihood of going to college
	...
	compress
		...
		sum *_lpred?
		save "$TEMP\likelihood_predicted_uni", replace
	restore
