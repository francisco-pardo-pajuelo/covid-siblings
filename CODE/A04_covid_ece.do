...
foreach size in "2_4" "2" "3" "4" {
	foreach v in "score_math_std" "score_com_std" "asp_not_school" "asp_college4" "socioec_index" {
		...
		*- Panel A: 2nd Grade
					//eststo `vlab'_2p_`size'	: reghdfe `v'_2p 		treated_post treated post fe		if sample_size==1 & sample_1==1, a(id_ie year)
					eststo `vlab'_2p_x_`size'	: reghdfe `v'_2p 		treated_post treated post fe ${x_all}	if sample_size==1 & sample_1==1, a(id_ie year)
				
		*- Panel B: 4th grade
					//eststo `vlab'_4p_`size'	: reghdfe `v'_4p 		treated_post treated post fe		if sample_size==1 & sample_2==1, a(id_ie year)
					eststo `vlab'_4p_x_`size'	: reghdfe `v'_4p 		treated_post treated post fe ${x_all}	if sample_size==1 & sample_2==1, a(id_ie year)
				

		*- Panel B census: 4th grade (only census)
					//eststo `vlab'_4p_c_`size'	: reghdfe `v'_4p 		treated_post treated post fe		if sample_size==1 & sample_3==1, a(id_ie year)
					//eststo `vlab'_4p_c_x_`size'	: reghdfe `v'_4p 		treated_post treated post fe ${x_all}	if sample_size==1 & sample_3==1, a(id_ie year)
			
		
		*- Panel C 8th grade (only those with potential baseline 2p scores (2014))
					//eststo `vlab'_2s_`size'			: 	reghdfe `v'_2s 			treated_post treated post fe							if sample_size==1 & sample_4==1, a(id_ie year)
					eststo `vlab'_2s_x_`size'			: 	reghdfe `v'_2s 			treated_post treated post fe ${x_all}						if sample_size==1 & sample_4==1, a(id_ie year)
					eststo `vlab'_2s_x_base_`size'		:	reghdfe `v'_2s 			treated_post treated post fe ${x_all} ${baseline_score_2p}	if sample_size==1 & sample_4==1, a(id_ie year)
