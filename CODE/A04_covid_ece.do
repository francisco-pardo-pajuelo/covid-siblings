...
program define main 
	...
	*-Pairs of ECE years with pre-post COVID
	twfe_ece siblings oldest
	twfe_ece siblings all
	twfe_ece_gender siblings oldest
	twfe_ece_gender siblings all
	
	twfe_gpa_baseline_survey_common	siblings oldest
	twfe_gpa_baseline_survey_common	siblings all
...
capture assert "`treatment_type'" == "siblings"
if _rc!=0 di "Need to select type: usual is -siblings- (only for now)"
...
gen sample_2 = 1 if grade==4 //2016,2018,2019 - 2022, 2023, 2024
...
foreach size in "2_4" "2" "3" "4" {
	foreach v in "score_math_std" "score_com_std" "asp_not_school" "asp_college4" "socioec_index" {
		...
		if "`size'" == "2_4" gen sample_size=1
		if "`size'" != "2_4" gen sample_size=inlist(fam_total_${fam_type},1,`size')==1

		if "`v'" == "score_math_std" 	local vlab = "ece_m"
		if "`v'" == "score_com_std" 	local vlab = "ece_r"
		if "`v'" == "asp_not_school" 	local vlab = "asp_ns"
		if "`v'" == "asp_college4" 		local vlab = "asp_c4"
		if "`v'" == "socioec_index" 	local vlab = "ses"
		
	
		di as result "*********************" _n as text "Size: `size'" _n  "Var: `v'" _n as result "*********************"	
		
		*- Panel A: 2nd Grade
					//eststo `vlab'_2p_`size'	: reghdfe `v'_2p 		treated_post treated post fe		if sample_size==1 & sample_1==1, a(year id_ie)  cluster(id_ie)
					eststo `vlab'_2p_x_`size'	: reghdfe `v'_2p 		treated_post treated post fe ${x_all}	if sample_size==1 & sample_1==1, a(year id_ie)  cluster(id_ie)
				
		*- Panel B: 4th grade
					//eststo `vlab'_4p_`size'	: reghdfe `v'_4p 		treated_post treated post fe		if sample_size==1 & sample_2==1, a(year id_ie)  cluster(id_ie)
					eststo `vlab'_4p_x_`size'	: reghdfe `v'_4p 		treated_post treated post fe ${x_all}	if sample_size==1 & sample_2==1, a(year id_ie)  cluster(id_ie)
				

		*- Panel B census: 4th grade (only census)
					//eststo `vlab'_4p_c_`size'	: reghdfe `v'_4p 		treated_post treated post fe		if sample_size==1 & sample_3==1, a(year id_ie)  cluster(id_ie)
					//eststo `vlab'_4p_c_x_`size'	: reghdfe `v'_4p 		treated_post treated post fe ${x_all}	if sample_size==1 & sample_3==1, a(year id_ie)  cluster(id_ie)
			
		
		*- Panel C 8th grade (only those with potential baseline 2p scores (2014))
					//eststo `vlab'_2s_`size'			: 	reghdfe `v'_2s 			treated_post treated post fe							if sample_size==1 & sample_4==1, a(year id_ie)  cluster(id_ie)
					eststo `vlab'_2s_x_`size'			: 	reghdfe `v'_2s 			treated_post treated post fe ${x_all}						if sample_size==1 & sample_4==1, a(year id_ie)  cluster(id_ie)
					eststo `vlab'_2s_x_base_`size'		:	reghdfe `v'_2s 			treated_post treated post fe ${x_all} ${baseline_score_2p}	if sample_size==1 & sample_4==1, a(year id_ie)  cluster(id_ie)
...
*- Figure ECE
	coefplot ///
			...
			bylabel("4th grade") ///
			|| ///
			(ece_m_2s_x_2_4, mcolor("${blue_1}") 	fcolor("${blue_1}%50")	lcolor("${blue_1}%50") 	ciopts(color("${blue_1}"))) ///
			(ece_r_2s_x_2_4, mcolor("${red_1}") 	fcolor("${red_1}%50")	lcolor("${red_1}%50") 	ciopts(color("${red_1}"))) ///
			, ///
			bylabel("8th grade") ///		
			|| ///
			, ///
		...
		if "${covid_data}" == "_TEST" {
			di "TEST"
			capture qui graph export "$FIGURES_TEMP\TWFE\twfe_ece_2_4_8_T`treatment_type'_S`subsample'_`other_filters'${max_sibs}${covid_data}.png", replace	
			capture qui graph export "$FIGURES_TEMP\TWFE\twfe_ece_2_4_8_T`treatment_type'_S`subsample'_`other_filters'${max_sibs}${covid_data}.pdf", replace	
			}
		if "${covid_data}" == "" {
			di "REAL"
...
*- Figure Expectations
	coefplot ///
			...
			bylabel("4th grade") ///
			|| ///
			(asp_c4_2s_x_2_4, mcolor(gs0) 	fcolor(gs0%20)	lcolor(gs0%20) 	ciopts(color(gs0))) ///
			, ///
			bylabel("8th grade") ///		
			|| ///
			, ///
		...
		if "${covid_data}" == "_TEST" {
			di "TEST"
			capture qui graph export "$FIGURES_TEMP\TWFE\twfe_expectation_2_4_8_T`treatment_type'_S`subsample'_`other_filters'${max_sibs}${covid_data}.png", replace	
			capture qui graph export "$FIGURES_TEMP\TWFE\twfe_expectation_2_4_8_T`treatment_type'_S`subsample'_`other_filters'${max_sibs}${covid_data}.pdf", replace	
			}
		if "${covid_data}" == "" {
			di "REAL"
			capture qui graph export "$FIGURES\TWFE\twfe_expectation_2_4_8_T`treatment_type'_S`subsample'_`other_filters'${max_sibs}${covid_data}.png", replace	
			capture qui graph export "$FIGURES\TWFE\twfe_expectation_2_4_8_T`treatment_type'_S`subsample'_`other_filters'${max_sibs}${covid_data}.pdf", replace
			}	
	...
	coefplot ///
			...
			bylabel("4th grade") ///
			|| ///
			(asp_c4_2s_x_2, mcolor("${blue_1}") 	fcolor("${blue_1}%50")	lcolor("${blue_1}%50") 	ciopts(color("${blue_1}"))) ///
			(asp_c4_2s_x_3, mcolor("${red_1}") 	fcolor("${red_1}%50")	lcolor("${red_1}%50") 	ciopts(color("${red_1}"))) ///
			(asp_c4_2s_x_4, mcolor("${green_1}") 	fcolor("${green_1}%50")	lcolor("${green_1}%50") 	ciopts(color("${green_1}"))) ///
			, ///
			bylabel("8th grade") ///		
			|| ///
			, ///
		...
		if "${covid_data}" == "_TEST" {
			di "TEST"
			capture qui graph export "$FIGURES_TEMP\TWFE\twfe_expectation_bysibs_2_4_8_T`treatment_type'_S`subsample'_`other_filters'${max_sibs}${covid_data}.png", replace	
			capture qui graph export "$FIGURES_TEMP\TWFE\twfe_expectation_bysibs_2_4_8_T`treatment_type'_S`subsample'_`other_filters'${max_sibs}${covid_data}.pdf", replace	
			}
		if "${covid_data}" == "" {
			di "REAL"
			capture qui graph export "$FIGURES\TWFE\twfe_expectation_bysibs_2_4_8_T`treatment_type'_S`subsample'_`other_filters'${max_sibs}${covid_data}.png", replace	
...
*- Figure Income
	coefplot ///
			...
			bylabel("4th grade") ///
			|| ///
			(ses_2s_x_2_4, mcolor(gs0) 	fcolor(gs0%20)	lcolor(gs0%20) 	ciopts(color(gs0))) ///
			, ///
			bylabel("8th grade") ///		
			|| ///
			, ///
		...
		if "${covid_data}" == "_TEST" {
			di "TEST"
			capture qui graph export "$FIGURES_TEMP\TWFE\twfe_ses_2_4_8_T`treatment_type'_S`subsample'_`other_filters'${max_sibs}${covid_data}.png", replace	
			capture qui graph export "$FIGURES_TEMP\TWFE\twfe_ses_2_4_8_T`treatment_type'_S`subsample'_`other_filters'${max_sibs}${covid_data}.pdf", replace	
			}
		if "${covid_data}" == "" {
			di "REAL"
			capture qui graph export "$FIGURES\TWFE\twfe_ses_2_4_8_T`treatment_type'_S`subsample'_`other_filters'${max_sibs}${covid_data}.png", replace	
			capture qui graph export "$FIGURES\TWFE\twfe_ses_2_4_8_T`treatment_type'_S`subsample'_`other_filters'${max_sibs}${covid_data}.pdf", replace
			}				
	...
	coefplot ///
			...
			bylabel("4th grade") ///
			|| ///
			(ses_2s_x_2, mcolor("${blue_1}") 	fcolor("${blue_1}%50")	lcolor("${blue_1}%50") 	ciopts(color("${blue_1}"))) ///
			(ses_2s_x_3, mcolor("${red_1}") 	fcolor("${red_1}%50")	lcolor("${red_1}%50") 	ciopts(color("${red_1}"))) ///
			(ses_2s_x_4, mcolor("${green_1}") 	fcolor("${green_1}%50")	lcolor("${green_1}%50") 	ciopts(color("${green_1}"))) ///
			, ///
			bylabel("8th grade") ///		
			|| ///
			, ///
		...
		if "${covid_data}" == "_TEST" {
			di "TEST"
			capture qui graph export "$FIGURES_TEMP\TWFE\twfe_ses_bysibs_2_4_8_T`treatment_type'_S`subsample'_`other_filters'${max_sibs}${covid_data}.png", replace	
			capture qui graph export "$FIGURES_TEMP\TWFE\twfe_ses_bysibs_2_4_8_T`treatment_type'_S`subsample'_`other_filters'${max_sibs}${covid_data}.pdf", replace	
			}
		if "${covid_data}" == "" {
			di "REAL"
...
****
	...
	****** TABLE HEADER
	...
	file write table_tex	/// HEADER OPTIONS OF TABLE
						...
						"	\centering" _n ///
						"	\caption{TWFE on Standardized Exams, Expectations and Socio-Economic Status}" _n ///
	...
	******* TABLE CONTENT	
	...
	file write table_tex ///	
					"&  &  &   \\" _n  ///
					"\multicolumn{4}{l}{\textit{Panel A: 2nd grade students}} \\" _n
	file close table_tex	
	
	estout   ece_m_2p_x_2 ece_m_2p_x_3 ece_m_2p_x_4 ///
	using "$TABLES\twfe_ece.tex", ///
	append style(tex) ///
	cells(b(star fmt(%9.3f)) se(par fmt(%9.3f))) ///
	keep(treated_post) ///
	varlabels(treated_post "\hspace{3mm}Mathematics") ///   //"\multirow{2}{*}{\shortstack[l]{Younger sibling born after \\ school-entry cutoff}}"
	...
	file open  table_tex	using "$TABLES\twfe_ece.tex", append write	
	file write table_tex ///	
						"%&  &  &   \\" _n  //Commenting with '%' because want to avoid this space
	file close table_tex		
	
	estout   ece_r_2p_x_2 ece_r_2p_x_3 ece_r_2p_x_4 ///
	using "$TABLES\twfe_ece.tex", ///
	append style(tex) ///
	cells(b(star fmt(%9.3f)) se(par fmt(%9.3f))) ///
	keep(treated_post) ///
	varlabels(treated_post "\hspace{3mm}Reading") ///   //"\multirow{2}{*}{\shortstack[l]{Younger sibling born after \\ school-entry cutoff}}"
	///indicate("School FE" = fe, labels("Yes" "No")) ///
	///stats(blank_line N   , fmt(%9.0fc %9.0fc ) labels(" " "\hspace{3mm}Observations")) ///
	mlabels(, none) collabels(, none) note(" ") label starlevels(* 0.10 ** 0.05 *** 0.01)
	...
	estout   asp_ns_2p_x_2 asp_ns_2p_x_3 asp_ns_2p_x_4 ///
	using "$TABLES\twfe_ece.tex", ///
	append style(tex) ///
	cells(b(star fmt(%9.3f)) se(par fmt(%9.3f))) ///
	keep(treated_post) ///
	varlabels(treated_post "\hspace{3mm}Max Expectation: Finish school") ///   //"\multirow{2}{*}{\shortstack[l]{Younger sibling born after \\ school-entry cutoff}}"
	///indicate("School FE" = fe, labels("Yes" "No")) ///
	///stats(blank_line N   , fmt(%9.0fc %9.0fc ) labels(" " "\hspace{3mm}Observations")) ///
	mlabels(, none) collabels(, none) note(" ") label starlevels(* 0.10 ** 0.05 *** 0.01)
	...
	estout   asp_c4_2p_x_2 asp_c4_2p_x_3 asp_c4_2p_x_4 ///
	using "$TABLES\twfe_ece.tex", ///
	append style(tex) ///
	cells(b(star fmt(%9.3f)) se(par fmt(%9.3f))) ///
	keep(treated_post) ///
	varlabels(treated_post "\hspace{3mm}Max Expectation: 4-year college") ///   //"\multirow{2}{*}{\shortstack[l]{Younger sibling born after \\ school-entry cutoff}}"
	///indicate("School FE" = fe, labels("Yes" "No")) ///
	///stats(blank_line N   , fmt(%9.0fc %9.0fc ) labels(" " "\hspace{3mm}Observations")) ///
	mlabels(, none) collabels(, none) note(" ") label starlevels(* 0.10 ** 0.05 *** 0.01)
	...
	estout   ses_2p_x_2 ses_2p_x_3 ses_2p_x_4 ///
	using "$TABLES\twfe_ece.tex", ///
	append style(tex) ///
	cells(b(star fmt(%9.3f)) se(par fmt(%9.3f))) ///
	keep(treated_post) ///
	varlabels(treated_post "\hspace{3mm}SES") ///   //"\multirow{2}{*}{\shortstack[l]{Younger sibling born after \\ school-entry cutoff}}"
	///indicate("School FE" = fe, labels("Yes" "No")) ///
	///stats(blank_line N   , fmt(%9.0fc %9.0fc ) labels(" " "\hspace{3mm}Observations")) ///
	mlabels(, none) collabels(, none) note(" ") label starlevels(* 0.10 ** 0.05 *** 0.01)	
	...
	file write table_tex ///	
					"&  &  &   \\" _n  ///
					"\multicolumn{4}{l}{\textit{Panel B: 4th grade students}} \\" _n
	file close table_tex	
	
	estout   ece_m_4p_x_2 ece_m_4p_x_3 ece_m_4p_x_4 ///
	using "$TABLES\twfe_ece.tex", ///
	append style(tex) ///
	cells(b(star fmt(%9.3f)) se(par fmt(%9.3f))) ///
	keep(treated_post) ///
	...
	estout   ece_r_4p_x_2 ece_r_4p_x_3 ece_r_4p_x_4 ///
	using "$TABLES\twfe_ece.tex", ///
	append style(tex) ///
	cells(b(star fmt(%9.3f)) se(par fmt(%9.3f))) ///
	keep(treated_post) ///
	...
	estout   asp_ns_4p_x_2 asp_ns_4p_x_3 asp_ns_4p_x_4 ///
	using "$TABLES\twfe_ece.tex", ///
	append style(tex) ///
	cells(b(star fmt(%9.3f)) se(par fmt(%9.3f))) ///
	keep(treated_post) ///
	...
	estout   asp_c4_4p_x_2 asp_c4_4p_x_3 asp_c4_4p_x_4 ///
	using "$TABLES\twfe_ece.tex", ///
	append style(tex) ///
	cells(b(star fmt(%9.3f)) se(par fmt(%9.3f))) ///
	keep(treated_post) ///
	...
	estout   ses_4p_x_2 ses_4p_x_3 ses_4p_x_4 ///
	using "$TABLES\twfe_ece.tex", ///
	append style(tex) ///
	cells(b(star fmt(%9.3f)) se(par fmt(%9.3f))) ///
	keep(treated_post) ///
	...
	file write table_tex ///	
					"&  &  &   \\" _n  ///
					"\multicolumn{4}{l}{\textit{Panel C: 8th grade students}} \\" _n
	file close table_tex	
	
	estout   ece_m_2s_x_base_2 ece_m_2s_x_base_3 ece_m_2s_x_base_4 ///
	using "$TABLES\twfe_ece.tex", ///
	append style(tex) ///
	cells(b(star fmt(%9.3f)) se(par fmt(%9.3f))) ///
	keep(treated_post) ///
	...
	estout   ece_r_2s_x_base_2 ece_r_2s_x_base_3 ece_r_2s_x_base_4 ///
	using "$TABLES\twfe_ece.tex", ///
	append style(tex) ///
	cells(b(star fmt(%9.3f)) se(par fmt(%9.3f))) ///
	keep(treated_post) ///
	...
	estout   asp_ns_2s_x_base_2 asp_ns_2s_x_base_3 asp_ns_2s_x_base_4 ///
	using "$TABLES\twfe_ece.tex", ///
	append style(tex) ///
	cells(b(star fmt(%9.3f)) se(par fmt(%9.3f))) ///
	keep(treated_post) ///
	...
	estout   asp_c4_2s_x_base_2 asp_c4_2s_x_base_3 asp_c4_2s_x_base_4 ///
	using "$TABLES\twfe_ece.tex", ///
	append style(tex) ///
	cells(b(star fmt(%9.3f)) se(par fmt(%9.3f))) ///
	keep(treated_post) ///
	...
	estout   ses_2s_x_base_2 ses_2s_x_base_3 ses_2s_x_base_4 ///
	using "$TABLES\twfe_ece.tex", ///
	append style(tex) ///
	cells(b(star fmt(%9.3f)) se(par fmt(%9.3f))) ///
	keep(treated_post) ///
...
keep id_per_umc id_ie year level grade male_siagie region_siagie public_siagie urban_siagie lives_with_mother lives_with_father approved approved_first std_gpa_m_adj std_gpa_c_adj
	...
	******* TABLE CONTENT				
	...
	file write table_tex ///	
					"&  &  & &  \\" _n  ///
					"\multicolumn{5}{l}{\textit{Panel A: GPA}} \\" _n
	file close table_tex	
	...
	file write table_tex ///	
					"&  &  & &  \\" _n  ///
					"\multicolumn{5}{l}{\textit{Panel B: Standardized Exams}} \\" _n
	file close table_tex	
...
args treatment_type subsample
	...
	foreach v in /*"std_gpa_m" "std_gpa_c"*/ "std_gpa_m_adj" "std_gpa_c_adj" /*"pass_math" "pass_read" "prim_on_time" "higher_ed_parent"*/ /*"approved" "approved_first"*/ {
		...
		estimates clear
			...
			//local size = "2_4"
				...
				//local subj = "m"
					eststo `v'_all1_`size'	: reghdfe `v' treated_post treated post, vce(cluster id_ie)
					eststo `v'_all2_`size'	: reghdfe `v' treated_post treated post ${x_all}, vce(cluster id_ie)
					eststo `v'_all3_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score}, vce(cluster id_ie)
					eststo `v'_all4_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses}, vce(cluster id_ie)
					
					//Effect by SES
					eststo `v'_ses1_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if base_socioec_index_cat==1, vce(cluster id_ie)
					eststo `v'_ses2_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if base_socioec_index_cat==2, vce(cluster id_ie)
					eststo `v'_ses3_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if base_socioec_index_cat==3, vce(cluster id_ie)
					eststo `v'_ses4_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if base_socioec_index_cat==4, vce(cluster id_ie)
					
					//Effect by Resources (overall))
					eststo `v'_resour1_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if /*base_internet==1 &*/ base_pc==0 & base_phone_internet==0, vce(cluster id_ie)
					eststo `v'_resour2_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if /*base_internet==1 &*/ base_pc==1 & base_phone_internet==0, vce(cluster id_ie)
					eststo `v'_resour3_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if /*base_internet==1 &*/ base_pc==0 & base_phone_internet==1, vce(cluster id_ie)
					eststo `v'_resour4_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if /*base_internet==1 &*/ base_pc==1 & base_phone_internet==1, vce(cluster id_ie)

					//Effect by Resources (TV)
					eststo `v'_tv0_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if base_tv==0, vce(cluster id_ie)
					eststo `v'_tv1_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if base_tv==1, vce(cluster id_ie)
						
					//Effect by Resources (Radio)
					eststo `v'_rad0_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if base_radio==0, vce(cluster id_ie)
					eststo `v'_rad1_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if base_radio==1, vce(cluster id_ie)
											
					//Effect by Resources (TV & Radio)
					eststo `v'_tvrad0_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if base_tv==0 & base_radio==0, vce(cluster id_ie)
					eststo `v'_tvrad1_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if base_tv==1 | base_radio==1, vce(cluster id_ie)					
					
					//Effect by Resources (TV & Radio Radio in Rural)
					eststo `v'_tvradrur0_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if base_tv==0 & base_radio==0 & urban_siagie==0, vce(cluster id_ie)
					eststo `v'_tvradrur1_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if (base_tv==1 | base_radio==1) & urban_siagie==0, vce(cluster id_ie)
						
					//Effect by Resources (PC + internet)
					eststo `v'_pc_int0_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if base_internet==0 & base_pc==0, vce(cluster id_ie)
					eststo `v'_pc_int1_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if base_internet==1 & base_pc==1, vce(cluster id_ie)
											
					//Effect by Resources (Phone with internet)
					eststo `v'_phoint0_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if base_phone_internet==0, vce(cluster id_ie)
					eststo `v'_phoint1_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if base_phone_internet==1, vce(cluster id_ie)					

					//Effect by Academic achievement
					eststo `v'_acad1_`size'		: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if base_score_acad_Q==1, vce(cluster id_ie)
					eststo `v'_acad2_`size'		: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if base_score_acad_Q==2, vce(cluster id_ie)
					eststo `v'_acad3_`size'		: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if base_score_acad_Q==3, vce(cluster id_ie)
					eststo `v'_acad4_`size'		: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if base_score_acad_Q==4, vce(cluster id_ie)
			
					//Effect by Expectations
					eststo `v'_asp_low_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if inlist(base_aspiration,1,2)==1, vce(cluster id_ie)
					//eststo `v'_asp_med_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if inlist(base_aspiration,3)==1, vce(cluster id_ie)
					eststo `v'_asp_hig_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if inlist(base_aspiration,4,5)==1, vce(cluster id_ie)
			
					//Effect by Expectations
					eststo `v'_asp2_low_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if inlist(base_aspiration,1,2,3,4)==1, vce(cluster id_ie)
					//eststo `v'_asp_med_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if inlist(base_aspiration,3)==1, vce(cluster id_ie)
					eststo `v'_asp2_hig_`size'	: reghdfe `v' treated_post treated post ${x_all} ${x_score} ${x_ses} if inlist(base_aspiration,5)==1, vce(cluster id_ie)				
			restore
			}
			...
			*- For Reviewing how adding controls affects the estimates
			coefplot ///
			(`v'_all1_2_4, mcolor(gs0) 	fcolor(gs0%20)	lcolor(gs0%20) 	ciopts(color(gs0))) ///
			, ///
			bylabel("No controls") ///
			|| ///
			(`v'_all2_2_4, mcolor(gs0) 	fcolor(gs0%20)	lcolor(gs0%20) 	ciopts(color(gs0))) ///
			, ///
			bylabel("Demographic") ///
			|| ///
			(`v'_all3_2_4, mcolor(gs0) 	fcolor(gs0%20)	lcolor(gs0%20) 	ciopts(color(gs0))) ///
			, ///
			bylabel("Demog + Academic") ///
			|| ///
			(`v'_all4_2_4, mcolor(gs0) 	fcolor(gs0%20)	lcolor(gs0%20) 	ciopts(color(gs0))) ///
			, ///
			bylabel("Demog + Academic + SES") ///
			|| ///
			, ///
			vertical ///
			recast(bar) barwidth(0.25) ///
			ciopts(recast(rcap)) citop ///
			xlabel(1 "No controls" 2 "Demographic" 3 "Demog + Academic" 4 "Demog + Academic + SES") ///	
			keep(treated_post) ///
			///legend(order(1 "1 sibling" 3 "2 siblings" 5 "3 siblings") col(3) pos(6)) ///
			legend(off) ///
		...
		if "${covid_data}" == "_TEST" {
			di "TEST"
			capture qui graph export "$FIGURES_TEMP\TWFE\twfe_`v'_bycontrols`sample_split'_T`treatment_type'_S`subsample'_pair`pair'_`other_filters'${max_sibs}${covid_data}.png", replace	
			capture qui graph export "$FIGURES_TEMP\TWFE\twfe_`v'_bycontrols`sample_split'_T`treatment_type'_S`subsample'_pair`pair'_`other_filters'${max_sibs}${covid_data}.pdf", replace	
			}
		if "${covid_data}" == "" {
			di "REAL"
			capture qui graph export "$FIGURES\TWFE\twfe_`v'_bycontrols`sample_split'_T`treatment_type'_S`subsample'_pair`pair'_`other_filters'${max_sibs}${covid_data}.png", replace	
			capture qui graph export "$FIGURES\TWFE\twfe_`v'_bycontrols`sample_split'_T`treatment_type'_S`subsample'_pair`pair'_`other_filters'${max_sibs}${covid_data}.pdf", replace
			}				
			
			coefplot ///
			(`v'_all1_2, mcolor("${blue_1}") 	fcolor("${blue_1}%50")	lcolor("${blue_1}%50") 	ciopts(color("${blue_1}"))) ///
			(`v'_all1_3, mcolor("${red_1}") 		fcolor("${red_1}%50")	lcolor("${red_1}%50") 	ciopts(color("${red_1}"))) ///
			(`v'_all1_4, mcolor("${green_1}") 	fcolor("${green_1}%50")	lcolor("${green_1}%50") 	ciopts(color("${green_1}"))) ///
			, ///
			bylabel("No controls") ///
			|| ///
			(`v'_all2_2, mcolor("${blue_1}") 	fcolor("${blue_1}%50")	lcolor("${blue_1}%50") 	ciopts(color("${blue_1}"))) ///
			(`v'_all2_3, mcolor("${red_1}") 		fcolor("${red_1}%50")	lcolor("${red_1}%50") 	ciopts(color("${red_1}"))) ///
			(`v'_all2_4, mcolor("${green_1}") 	fcolor("${green_1}%50")	lcolor("${green_1}%50") 	ciopts(color("${green_1}"))) ///
			, ///
			bylabel("Demographic") ///
			|| ///
			(`v'_all3_2, mcolor("${blue_1}") 	fcolor("${blue_1}%50")	lcolor("${blue_1}%50") 	ciopts(color("${blue_1}"))) ///
			(`v'_all3_3, mcolor("${red_1}") 		fcolor("${red_1}%50")	lcolor("${red_1}%50") 	ciopts(color("${red_1}"))) ///
			(`v'_all3_4, mcolor("${green_1}") 	fcolor("${green_1}%50")	lcolor("${green_1}%50") 	ciopts(color("${green_1}"))) ///
			, ///
			bylabel("Demog + Academic") ///
			|| ///
			(`v'_all4_2, mcolor("${blue_1}") 	fcolor("${blue_1}%50")	lcolor("${blue_1}%50") 	ciopts(color("${blue_1}"))) ///
			(`v'_all4_3, mcolor("${red_1}") 		fcolor("${red_1}%50")	lcolor("${red_1}%50") 	ciopts(color("${red_1}"))) ///
			(`v'_all4_4, mcolor("${green_1}") 	fcolor("${green_1}%50")	lcolor("${green_1}%50") 	ciopts(color("${green_1}"))) ///
			, ///
			bylabel("Demog + Academic + SES") ///
			|| ///
			, ///
		...
		if "${covid_data}" == "_TEST" {
			di "TEST"
			capture qui graph export "$FIGURES_TEMP\TWFE\twfe_`v'_bycontrols_bysibs_T`treatment_type'_S`subsample'_pair`pair'_`other_filters'${max_sibs}${covid_data}.png", replace	
			capture qui graph export "$FIGURES_TEMP\TWFE\twfe_`v'_bycontrols_bysibs_T`treatment_type'_S`subsample'_pair`pair'_`other_filters'${max_sibs}${covid_data}.pdf", replace	
			}
		if "${covid_data}" == "" {
			di "REAL"
			capture qui graph export "$FIGURES\TWFE\twfe_`v'_bycontrols_bysibs_T`treatment_type'_S`subsample'_pair`pair'_`other_filters'${max_sibs}${covid_data}.png", replace	
			capture qui graph export "$FIGURES\TWFE\twfe_`v'_bycontrols_bysibs_T`treatment_type'_S`subsample'_pair`pair'_`other_filters'${max_sibs}${covid_data}.pdf", replace
			}
	
			
			*- For heterogeneity with different sample splits (2 groups)
			foreach sample_split in  "ses"  "tv" "rad" "tvrad" "tvradrur" "phoint" "pc_int" "acad" "asp" "asp2" {
				if "`sample_split'" == "ses" {
					local coef0 = "ses1"
					local coef1 = "ses4"
					local coef0_lab = "Low SES Households (Q1)"
				...
				if "`sample_split'" == "pc_int" {
					local coef0 = "pc_int0"
					local coef1 = "pc_int1"
					local coef0_lab = "Households with neither PC nor Internet"
				...
				if "`sample_split'" == "acad" {
					local coef0 = "acad1"
					local coef1 = "acad4"
					local coef0_lab = "Low achievement students (Q1)"
					local coef1_lab = "High achievement students (Q4)"
				} 
				
				if "`sample_split'" == "asp" {
					local coef0 = "asp_low"
					local coef1 = "asp_hig"
					local coef0_lab = "Expectations: Below 4-year college"
					local coef1_lab = "Expectations: 4-year college and beyond"
				} 				
	
				if "`sample_split'" == "asp2" {
					local coef0 = "asp2_low"
					local coef1 = "asp2_hig"
					local coef0_lab = "Expectations: 4-year college or less"
					local coef1_lab = "Expectations: Masters or PhD"
				} 				

			coefplot ///
			(`v'_`coef0'_2_4, mcolor(gs0) 	fcolor(gs0%20)	lcolor(gs0%20) 	ciopts(color(gs0))) ///
			, ///
			bylabel("`coef0_lab'") ///
			|| ///
			(`v'_`coef1'_2_4, mcolor(gs0) 	fcolor(gs0%20)	lcolor(gs0%20) 	ciopts(color(gs0))) ///
			, ///
			bylabel("`coef1_lab'") ///
			|| ///
			, ///
			...
			recast(bar) barwidth(0.25) ///
			ciopts(recast(rcap)) citop ///
			xlabel(1 "`coef0_lab'" 2 "`coef1_lab'") ///	
			keep(treated_post) ///
			///legend(order(1 "1 sibling" 3 "2 siblings" 5 "3 siblings") col(3) pos(6)) ///
			legend(off) ///
			ytitle("Standardized Mathematics GPA" "relative to Only Children", size(medsmall) height(5)) ///							
		...
		if "${covid_data}" == "_TEST" {
			di "TEST"
			capture qui graph export "$FIGURES_TEMP\TWFE\twfe_`v'_by`sample_split'_T`treatment_type'_S`subsample'_pair`pair'_`other_filters'${max_sibs}${covid_data}.png", replace	
			capture qui graph export "$FIGURES_TEMP\TWFE\twfe_`v'_by`sample_split'_T`treatment_type'_S`subsample'_pair`pair'_`other_filters'${max_sibs}${covid_data}.pdf", replace	
			}
		if "${covid_data}" == "" {
			di "REAL"
			capture qui graph export "$FIGURES\TWFE\twfe_`v'_by`sample_split'_T`treatment_type'_S`subsample'_pair`pair'_`other_filters'${max_sibs}${covid_data}.png", replace	
			capture qui graph export "$FIGURES\TWFE\twfe_`v'_by`sample_split'_T`treatment_type'_S`subsample'_pair`pair'_`other_filters'${max_sibs}${covid_data}.pdf", replace
			}				
			
			coefplot ///
			(`v'_`coef0'_2, mcolor("${blue_1}") 	fcolor("${blue_1}%50")	lcolor("${blue_1}%50") 	ciopts(color("${blue_1}"))) ///
			(`v'_`coef0'_3, mcolor("${red_1}") 		fcolor("${red_1}%50")	lcolor("${red_1}%50") 	ciopts(color("${red_1}"))) ///
			(`v'_`coef0'_4, mcolor("${green_1}") 	fcolor("${green_1}%50")	lcolor("${green_1}%50") 	ciopts(color("${green_1}"))) ///
			, ///
			bylabel("`coef0_lab'") ///
			|| ///
			(`v'_`coef1'_2, mcolor("${blue_1}") 	fcolor("${blue_1}%50")	lcolor("${blue_1}%50") 	ciopts(color("${blue_1}"))) ///
			(`v'_`coef1'_3, mcolor("${red_1}") 		fcolor("${red_1}%50")	lcolor("${red_1}%50") 	ciopts(color("${red_1}"))) ///
			(`v'_`coef1'_4, mcolor("${green_1}") 	fcolor("${green_1}%50")	lcolor("${green_1}%50") 	ciopts(color("${green_1}"))) ///
			, ///
			bylabel("`coef1_lab'") ///
			|| ///
			, ///
		...
		if "${covid_data}" == "_TEST" {
			di "TEST"
			capture qui graph export "$FIGURES_TEMP\TWFE\twfe_`v'_by`sample_split'_bysibs_T`treatment_type'_S`subsample'_pair`pair'_`other_filters'${max_sibs}${covid_data}.png", replace	
			capture qui graph export "$FIGURES_TEMP\TWFE\twfe_`v'_by`sample_split'_bysibs_T`treatment_type'_S`subsample'_pair`pair'_`other_filters'${max_sibs}${covid_data}.pdf", replace	
			}
		if "${covid_data}" == "" {
			di "REAL"
			capture qui graph export "$FIGURES\TWFE\twfe_`v'_by`sample_split'_bysibs_T`treatment_type'_S`subsample'_pair`pair'_`other_filters'${max_sibs}${covid_data}.png", replace	
			capture qui graph export "$FIGURES\TWFE\twfe_`v'_by`sample_split'_bysibs_T`treatment_type'_S`subsample'_pair`pair'_`other_filters'${max_sibs}${covid_data}.pdf", replace
			}
		} /// Sample Split
		...
		*- For heterogeneity with different sample splits (4 groups)
		foreach sample_split in  "ses" /*"pc_int"*/ "acad" /*"asp"*/ "resour" {
			...
			if "`sample_split'" == "ses" {
				...
				local coef3 = "ses3"
				local coef4 = "ses4"
				local coef1_lab = "Low SES Households (Q1)"
				local coef2_lab = "Q2"
				local coef3_lab = "Q3"
				local coef4_lab = "High SES Households (Q4)"
			} 	
			...
			if "`sample_split'" == "acad" {
				...
				local coef3 = "acad3"
				local coef4 = "acad4"
				local coef1_lab = "Low achievement students (Q1)"
				local coef2_lab = "Q2"
				local coef3_lab = "Q3"
				local coef4_lab = "High achievement students (Q4)"
			}
			
			//local v = "std_gpa_m_adj"
			//local sample_split = "resour"
			if "`sample_split'" == "resour" {
				local coef1 = "resour1"
				local coef2 = "resour2"
				local coef3 = "resour3"
				local coef4 = "resour4"
				//local coef1_lab = "No PC nor phone with internet"
				local coef1_lab = `""No PC nor phone" "with internet""'
				
				local coef2_lab =  `""Only PC" "with internet""'
				local coef3_lab =  `""Only Phone" "with internet""'
				local coef4_lab =  `""PC and phone" "with internet""'
			}
			...
			*/
			...
			bylabel("`coef1_lab'") ///
			|| ///
			(`v'_`coef2'_2_4, mcolor(gs0) 	fcolor(gs0%20)	lcolor(gs0%20) 	ciopts(color(gs0))) ///
			, ///
			bylabel("`coef2_lab'") ///
			|| ///
			(`v'_`coef3'_2_4, mcolor(gs0) 	fcolor(gs0%20)	lcolor(gs0%20) 	ciopts(color(gs0))) ///
			, ///
			bylabel("`coef3_lab'") ///
			|| ///
			(`v'_`coef4'_2_4, mcolor(gs0) 	fcolor(gs0%20)	lcolor(gs0%20) 	ciopts(color(gs0))) ///
			, ///
			bylabel("`coef4_lab'") ///
			|| ///
			, ///
			...
			recast(bar) barwidth(0.25) ///
			ciopts(recast(rcap)) citop ///
			xlabel(1 `"`coef1_lab'"' 2 `"`coef2_lab'"' 3 `"`coef3_lab'"' 4 `"`coef4_lab'"') ///	
			keep(treated_post) ///
			///legend(order(1 "1 sibling" 3 "2 siblings" 5 "3 siblings") col(3) pos(6)) ///
			legend(off) ///
			ytitle("Standardized Mathematics GPA" "relative to Only Children", size(medsmall) height(5)) ///							
		...
		if "${covid_data}" == "_TEST" {
			di "TEST"
			capture qui graph export "$FIGURES_TEMP\TWFE\twfe_`v'_by`sample_split'4_T`treatment_type'_S`subsample'_pair`pair'_`other_filters'${max_sibs}${covid_data}.png", replace	
			capture qui graph export "$FIGURES_TEMP\TWFE\twfe_`v'_by`sample_split'4_T`treatment_type'_S`subsample'_pair`pair'_`other_filters'${max_sibs}${covid_data}.pdf", replace	
			}
		if "${covid_data}" == "" {
			di "REAL"
			capture qui graph export "$FIGURES\TWFE\twfe_`v'_by`sample_split'4_T`treatment_type'_S`subsample'_pair`pair'_`other_filters'${max_sibs}${covid_data}.png", replace	
			capture qui graph export "$FIGURES\TWFE\twfe_`v'_by`sample_split'4_T`treatment_type'_S`subsample'_pair`pair'_`other_filters'${max_sibs}${covid_data}.pdf", replace
			}				
			...
			bylabel("`coef1_lab'") ///
			|| ///
			(`v'_`coef2'_2, mcolor("${blue_1}") 	fcolor("${blue_1}%50")	lcolor("${blue_1}%50") 	ciopts(color("${blue_1}"))) ///
			(`v'_`coef2'_3, mcolor("${red_1}") 		fcolor("${red_1}%50")	lcolor("${red_1}%50") 	ciopts(color("${red_1}"))) ///
			(`v'_`coef2'_4, mcolor("${green_1}") 	fcolor("${green_1}%50")	lcolor("${green_1}%50") 	ciopts(color("${green_1}"))) ///
			, ///
			bylabel("`coef2_lab'") ///
			|| ///
			(`v'_`coef3'_2, mcolor("${blue_1}") 	fcolor("${blue_1}%50")	lcolor("${blue_1}%50") 	ciopts(color("${blue_1}"))) ///
			(`v'_`coef3'_3, mcolor("${red_1}") 		fcolor("${red_1}%50")	lcolor("${red_1}%50") 	ciopts(color("${red_1}"))) ///
			(`v'_`coef3'_4, mcolor("${green_1}") 	fcolor("${green_1}%50")	lcolor("${green_1}%50") 	ciopts(color("${green_1}"))) ///
			, ///
			bylabel("`coef3_lab'") ///
			|| ///
			(`v'_`coef4'_2, mcolor("${blue_1}") 	fcolor("${blue_1}%50")	lcolor("${blue_1}%50") 	ciopts(color("${blue_1}"))) ///
			(`v'_`coef4'_3, mcolor("${red_1}") 		fcolor("${red_1}%50")	lcolor("${red_1}%50") 	ciopts(color("${red_1}"))) ///
			(`v'_`coef4'_4, mcolor("${green_1}") 	fcolor("${green_1}%50")	lcolor("${green_1}%50") 	ciopts(color("${green_1}"))) ///
			, ///
			bylabel("`coef4_lab'") ///
			|| ///
			, ///
		...
		if "${covid_data}" == "_TEST" {
			di "TEST"
			capture qui graph export "$FIGURES_TEMP\TWFE\twfe_`v'_by`sample_split'4_bysibs_T`treatment_type'_S`subsample'_pair`pair'_`other_filters'${max_sibs}${covid_data}.png", replace	
			capture qui graph export "$FIGURES_TEMP\TWFE\twfe_`v'_by`sample_split'4_bysibs_T`treatment_type'_S`subsample'_pair`pair'_`other_filters'${max_sibs}${covid_data}.pdf", replace	
			}
		if "${covid_data}" == "" {
			di "REAL"
			capture qui graph export "$FIGURES\TWFE\twfe_`v'_by`sample_split'4_bysibs_T`treatment_type'_S`subsample'_pair`pair'_`other_filters'${max_sibs}${covid_data}.png", replace	
			capture qui graph export "$FIGURES\TWFE\twfe_`v'_by`sample_split'4_bysibs_T`treatment_type'_S`subsample'_pair`pair'_`other_filters'${max_sibs}${covid_data}.pdf", replace
			}
		} // Sample Split		

		coefplot 	(`v'_all3_2, mcolor("${ek_blue}") 		ciopts(color("${ek_blue}"))) ///
					(`v'_all3_3, mcolor("${ek_green}") 	ciopts(color("${ek_green}"))) ///
					(`v'_all3_4, mcolor("${ek_red}") 		ciopts(color("${ek_red}"))) ///
					, ///
		...
		if "${covid_data}" == "_TEST" {
			capture qui graph export "$FIGURES_TEMP\TWFE\twfe_`v'_baseline_survey_T`treatment_type'_S`subsample'_pair`pair'_${max_sibs}${covid_data}.png", replace	
			capture qui graph export "$FIGURES_TEMP\TWFE\twfe_`v'_baseline_survey_T`treatment_type'_S`subsample'_pair`pair'_${max_sibs}${covid_data}.pdf", replace	
			}
		if "${covid_data}" == "" {
			capture qui graph export "$FIGURES\TWFE\twfe_`v'_baseline_survey_T`treatment_type'_S`subsample'_pair`pair'_${max_sibs}${covid_data}.png", replace	
			capture qui graph export "$FIGURES\TWFE\twfe_`v'_baseline_survey_T`treatment_type'_S`subsample'_pair`pair'_${max_sibs}${covid_data}.pdf", replace	
			}	
	...
	****
		...
		capture erase "$TABLES\twfe_`v'_baseline_survey_1_T`treatment_type'_S`subsample'_1_pair`pair'${covid_data}.tex"
		capture erase "$TABLES\twfe_`v'_baseline_survey_2_T`treatment_type'_S`subsample'_1_pair`pair'${covid_data}.tex"
		
		****** TABLE HEADER
		
		file open  table_tex	using "$TABLES\twfe_`v'_baseline_survey_1_T`treatment_type'_S`subsample'_pair`pair'${covid_data}.tex", replace write
		file write table_tex	/// HEADER OPTIONS OF TABLE
							"\makeatletter" _n ///
							"\@ifclassloaded{beamer}{%" _n ///
							"	\centering" _n ///
							...
							"	\label{tab:twfe_`v'_baseline_survey_1_T`treatment_type'_S`subsample'_pair`pair'}" _n ///
		...
		file open  table_tex	using "$TABLES\twfe_`v'_baseline_survey_1_T`treatment_type'_S`subsample'_pair`pair'${covid_data}.tex", append write
		file write table_tex	/// HEADER OPTIONS OF TABLE
						"\begin{tabular}{lccc}" _n ///
						/// HEADER OF TABLE
						"\toprule" _n ///
		...
		******* TABLE CONTENT	
		...
		file write table_tex ///	
						"&  &  &   \\" _n  ///
						"\multicolumn{4}{l}{\textit{Panel A: All students}} \\" _n
		file close table_tex	
		

		estout   `v'_all3_2 `v'_all3_3 `v'_all3_4  ///
		using "$TABLES\twfe_`v'_baseline_survey_1_T`treatment_type'_S`subsample'_pair`pair'${covid_data}.tex", ///
		append style(tex) ///
		cells(b(star fmt(%9.3f)) se(par fmt(%9.3f))) ///
		keep(treated_post) ///
...
****
	...
	capture erase "$TABLES\twfe_`v'_baseline_survey_2_T`treatment_type'_S`subsample'_pair`pair'${covid_data}.tex"
	
	****** TABLE HEADER
	
	file open  table_tex	using "$TABLES\twfe_`v'_baseline_survey_2_T`treatment_type'_S`subsample'_pair`pair'${covid_data}.tex", replace write
	file write table_tex	/// HEADER OPTIONS OF TABLE
						"\makeatletter" _n ///
						"\@ifclassloaded{beamer}{%" _n ///
						"	\centering" _n ///
						...
						"	\label{tab:twfe_`v'_baseline_survey_2_T`treatment_type'_S`subsample'_pair`pair'}" _n ///
						"	\resizebox{0.65\textwidth}{!}%" _n ///
						"}" _n ///
						"{" _n ///
						"\makeatother"	 _n 
	...
	file open  table_tex	using "$TABLES\twfe_`v'_baseline_survey_2_T`treatment_type'_S`subsample'_pair`pair'${covid_data}.tex", append write
	file write table_tex	/// HEADER OPTIONS OF TABLE
					"\begin{tabular}{lccc}" _n ///
					/// HEADER OF TABLE
					"\toprule" _n ///
	...
	******* TABLE CONTENT	
	...
	estout   `v'_all3_2 `v'_all3_3 `v'_all3_4  ///
	using "$TABLES\twfe_`v'_baseline_survey_2_T`treatment_type'_S`subsample'_pair`pair'${covid_data}.tex", ///
	append style(tex) ///
	cells(b(star fmt(%9.3f)) se(par fmt(%9.3f))) ///
	keep(treated_post) ///
	varlabels(treated_post "\hspace{3mm}") ///   //"\multirow{2}{*}{\shortstack[l]{Younger sibling born after \\ school-entry cutoff}}"
	...
	********* END TABLE
	...
	/*
		...
		****** TABLE HEADER
		...
		file open  table_tex	using "$TABLES\twfe_gpa_baseline_survey_1_pair`pair'.tex", replace write
		file write table_tex	/// HEADER OPTIONS OF TABLE
							"\makeatletter" _n ///
							"\@ifclassloaded{beamer}{%" _n ///
							"	\centering" _n ///
							...
							"	\centering" _n ///
							"	\caption{`title_t1_pair_`pair''}" _n ///
							"	\label{tab:twfe_gpa_baseline_survey_1_pair`pair'}" _n ///
							"	\resizebox{0.65\textwidth}{!}%" _n ///
							"}" _n ///
							"{" _n ///
							"\makeatother"	 _n 
		...
		******* TABLE CONTENT	
		...
		using "$TABLES\twfe_gpa_baseline_survey_1_pair`pair'.tex", ///
		append style(tex) ///
		cells(b(star fmt(%9.3f)) se(par fmt(%9.3f))) ///
		keep(treated_post) ///
		varlabels(treated_post "\hspace{3mm}Mathematics") ///   //"\multirow{2}{*}{\shortstack[l]{Younger sibling born after \\ school-entry cutoff}}"
...
****
	...
	capture erase "$TABLES\twfe_gpa_baseline_survey_2_pair`pair'.tex"
	
	****** TABLE HEADER
	
	file open  table_tex	using "$TABLES\twfe_gpa_baseline_survey_2_pair`pair'.tex", replace write
	file write table_tex	/// HEADER OPTIONS OF TABLE
						"\makeatletter" _n ///
						"\@ifclassloaded{beamer}{%" _n ///
						"	\centering" _n ///
						...
						"	\centering" _n ///
						"	\caption{`title_t2_pair_`pair''}" _n ///
						"	\label{tab:twfe_gpa_baseline_survey_2_pair`pair'}" _n ///
						"	\resizebox{0.65\textwidth}{!}%" _n ///
						"}" _n ///
						"{" _n ///
						"\makeatother"	 _n 
	...
	******* TABLE CONTENT	
	...
	estout   gpa_m_all_2 gpa_m_all_3 gpa_m_all_4  ///
	using "$TABLES\twfe_gpa_baseline_survey_2_pair`pair'.tex", ///
	append style(tex) ///
	cells(b(star fmt(%9.3f)) se(par fmt(%9.3f))) ///
	keep(treated_post) ///
	varlabels(treated_post "\hspace{3mm}Mathematics") ///   //"\multirow{2}{*}{\shortstack[l]{Younger sibling born after \\ school-entry cutoff}}"
...
program define twfe_gpa_baseline_survey_unique
	...
	*/
					...
					//Parent's education
					eststo `v'_edu1_`size'		: reghdfe `v' treated_post treated post ${x_score} ${x_ses} if inlist(base_edu_mother_2p,1,2,3,4)==1			& pair==1, vce(cluster id_ie)
					eststo `v'_edu2_`size'		: reghdfe `v' treated_post treated post ${x_score} ${x_ses} if inlist(base_edu_mother_2p,4)==1					& pair==1, vce(cluster id_ie)
					eststo `v'_edu3_`size'		: reghdfe `v' treated_post treated post ${x_score} ${x_ses} if inlist(base_edu_mother_2p,5,6,7,8,9,10,11,12)==1	& pair==1, vce(cluster id_ie)
												
					//Books
					eststo `v'_book0_`size'		: reghdfe `v' treated_post treated post ${x_score} ${x_ses} if inlist(base_books_cat_2p,1,2)==1					& pair==1, vce(cluster id_ie)
					eststo `v'_book1_`size'		: reghdfe `v' treated_post treated post ${x_score} ${x_ses} if inlist(base_books_cat_2p,3,4,5,6,7)==1			& pair==1, vce(cluster id_ie)
					
					//Quiet room
					eststo `v'_quiet0_`size'	: reghdfe `v' treated_post treated post ${x_score} ${x_ses} if base_quiet_room_2p==0 & base_pc==0				& pair==1, vce(cluster id_ie)
					eststo `v'_quiet1_`size'	: reghdfe `v' treated_post treated post ${x_score} ${x_ses} if base_quiet_room_2p==1 & base_pc==1				& pair==1, vce(cluster id_ie)
			
					//By years in Pre-K
					eststo `v'_prek0_`size'		: reghdfe `v' treated_post treated post ${x_score} ${x_ses} if inlist(base_years_prek_2p,1,2,3,5)==1				& pair==1, vce(cluster id_ie)
					eststo `v'_prek1_`size'		: reghdfe `v' treated_post treated post ${x_score} ${x_ses} if inlist(base_years_prek_2p,4)==1				& pair==1, vce(cluster id_ie)
			
					//By grade repetition
					eststo `v'_rep0_`size'		: reghdfe `v' treated_post treated post ${x_score} ${x_ses} if base_has_repeated_2s==1							& pair==4, vce(cluster id_ie)
					eststo `v'_rep1_`size'		: reghdfe `v' treated_post treated post ${x_score} ${x_ses} if base_has_repeated_2s==2							& pair==4, vce(cluster id_ie)
...
program define event_ece_student	
	...
	*- ECE - 2P
	foreach v in "score_math_2p" "score_com_2p" "score_math_std_2p" "score_com_std_2p" "score_acad_std_2p" "satisf_m_2p" "satisf_c_2p" {
		...
		reghdfe 	`v' ${x}	year_t_b6 year_t_b5		year_t_b4 			year_t_b2 o.year_t_o1 year_t_a2 year_t_a3 i.year treated, a(id_ie) vce(cluster id_ie)
		*reghdfe 	score_acad_std_2p	year_t_b6 year_t_b5		year_t_b4 			year_t_b2 o.year_t_o1 year_t_a2 year_t_a3 i.year treated if score_acad_std_2p_max==2022 & score_acad_std_2p_sum==5 & fam_order_2==1, a(id_ie) vce(cluster id_ie)
		estimates 	store `v'
		}
	...
	*- ECE - 4P
	foreach v in "score_math_4p" "score_com_4p" "score_math_std_4p" "score_com_std_4p" "score_acad_std_4p" "satisf_m_4p" "satisf_c_4p" {
		...
		reghdfe 	`v' 	${x}						year_t_b4 			year_t_b2 o.year_t_o1 year_t_a2 year_t_a3 year_t_a4  treated, a(year id_ie)  cluster(id_ie)
		estimates 	store `v'		
		}	
	...
	*- ECE - 2S
	foreach v in "score_math_2s" "score_com_2s" "score_math_std_2s" "score_com_std_2s" "score_acad_std_2s" "satisf_m_2s" "satisf_c_2s" {
		...
		keep if inlist(year,2015,2016,2018,2019,2022,2023)==1
		reghdfe 	`v' ${x}			 year_t_b5 		year_t_b4  			year_t_b2 o.year_t_o1 year_t_a2 year_t_a3  treated, a(year id_ie)  cluster(id_ie)
		estimates 	store `v'		
		}	
	...
	estimates clear	
		*- ECE - 2P
		foreach v in "score_math_2p" "score_com_2p" "score_math_std_2p" "score_com_std_2p" "score_acad_std_2p" "satisf_m_2p" "satisf_c_2p" {
			...
			if "`weight'" == "" 		reghdfe 	`v' /*${x}*/	year_t_b6 year_t_b5		year_t_b4 			year_t_b2 o.year_t_o1 year_t_a2 year_t_a3 i.year treated, a(id_ie) vce(cluster id_ie)
			if "`weight'" == "_weighted" 	reghdfe 	`v' /*${x}*/	year_t_b6 year_t_b5		year_t_b4 			year_t_b2 o.year_t_o1 year_t_a2 year_t_a3 i.year treated [aw=pop], a(id_ie) vce(cluster id_ie)
			estimates 	store `v'
			}
		...
		*- ECE - 4P
		foreach v in "score_math_4p" "score_com_4p" "score_math_std_4p" "score_com_std_4p" "score_acad_std_4p" "satisf_m_4p" "satisf_c_4p" {
			...
			if "`weight'" == "" 		reghdfe 	`v' /*${x}*/	 		year_t_b4 			year_t_b2 o.year_t_o1 year_t_a2 year_t_a3 year_t_a4 i.year treated, a(id_ie) vce(cluster id_ie)
			if "`weight'" == "_weighted" 	reghdfe 	`v' /*${x}*/	 		year_t_b4 			year_t_b2 o.year_t_o1 year_t_a2 year_t_a3 year_t_a4 i.year treated [aw=pop], a(id_ie) vce(cluster id_ie)		
			estimates 	store `v'		
			}	
		...
		*- ECE - 2S
		foreach v in "score_math_2s" "score_com_2s" "score_math_std_2s" "score_com_std_2s" "score_acad_std_2s" "satisf_m_2s" "satisf_c_2s" {
			...
			if "`weight'" == "" 		reghdfe 	`v' /*${x}*/	 year_t_b5		year_t_b4 			year_t_b2 o.year_t_o1 year_t_a2 year_t_a3 i.year treated, a(id_ie) vce(cluster id_ie)
			if "`weight'" == "_weighted" 	reghdfe 	`v' /*${x}*/	 year_t_b5		year_t_b4 			year_t_b2 o.year_t_o1 year_t_a2 year_t_a3 i.year treated [aw=pop], a(id_ie) vce(cluster id_ie)					
			estimates 	store `v'
			}	
...
*/	
	...
	reghdfe satisf_m i.fam_total_${fam_type} i.year i.educ_mother i.educ_father if grade==2, a(id_ie) vce(cluster id_ie)
	reghdfe satisf_m i.fam_total_${fam_type} i.year i.educ_mother i.educ_father if grade==4, a(id_ie) vce(cluster id_ie)
	reghdfe satisf_m i.fam_total_${fam_type} i.year i.educ_mother i.educ_father if grade==8, a(id_ie) vce(cluster id_ie)
...
capture program drop twfe_ece_gender
program define twfe_ece_gender	
...
gen treated_post_male = treated_post * male_siagie
gen post_male = post*male_siagie
gen treated_male  = treated*male_siagie

foreach size in "2_4" /*"2" "3" "4"*/ {
	foreach v in "score_math" "score_com" /*"score_math_std" "score_com_std"*/ {
		di "******" _n "size:`size' / var: `var'" _n "*********"
		
		if "`size'" == "2_4" gen sample_size=1
		if "`size'" != "2_4" gen sample_size=inlist(fam_total_${fam_type},1,`size')==1
		
		if "`v'" == "score_math" 	local vlab = "ece_m"
		if "`v'" == "score_math_std" 	local vlab = "ece_m"
		if "`v'" == "score_com" 	local vlab = "ece_r"
		if "`v'" == "score_com_std" 	local vlab = "ece_r"
		if "`v'" == "asp_not_school" 	local vlab = "asp_ns"
		if "`v'" == "asp_college4" 		local vlab = "asp_c4"
		if "`v'" == "socioec_index" 	local vlab = "ses"		
		...
		eststo `vlab'_2p_x_`size'	: reghdfe `v'_2p 		treated_post treated post fe ${x_all}	if sample_size==1 & sample_1==1, a(year id_ie)  cluster(id_ie)
		eststo `vlab'_2p_x_`size'_m	: reghdfe `v'_2p 		treated_post treated post fe ${x_all}	if sample_size==1 & sample_1==1 & male_siagie==1, a(year id_ie)  cluster(id_ie)
		eststo `vlab'_2p_x_`size'_f	: reghdfe `v'_2p 		treated_post treated post fe ${x_all}	if sample_size==1 & sample_1==1 & male_siagie==0, a(year id_ie)  cluster(id_ie)
		eststo `vlab'_2p_x_`size'_mf: reghdfe `v'_2p 		treated_post treated_post_male treated post treated_male post_male fe ${x_all}	if sample_size==1 & sample_1==1, a(year id_ie)  cluster(id_ie)
		mat mat_`vlab'_2p = r(table)	
		
		eststo `vlab'_4p_x_`size'	:reghdfe `v'_4p 	treated_post treated post fe ${x_all}	if sample_size==1 & sample_2==1, a(year id_ie)  cluster(id_ie)
		eststo `vlab'_4p_x_`size'_m	:reghdfe `v'_4p		treated_post treated post fe ${x_all}	if sample_size==1 & sample_2==1 & male_siagie==1, a(year id_ie)  cluster(id_ie)
		eststo `vlab'_4p_x_`size'_f	:reghdfe `v'_4p		treated_post treated post fe ${x_all}	if sample_size==1 & sample_2==1 & male_siagie==0, a(year id_ie)  cluster(id_ie)
		eststo `vlab'_4p_x_`size'_mf:reghdfe `v'_4p		treated_post treated_post_male treated post treated_male post_male ${x_all}	if sample_size==1 & sample_2==1, a(year id_ie)  cluster(id_ie)
		mat mat_`vlab'_4p = r(table)	
			
		foreach g in "2p" "4p" {
		if mat_`vlab'_`g'[4,2] < 0.01 {
			local stars "***"
		}
		else if mat_`vlab'_`g'[4,2] < 0.05 {
			local stars "**"
		}
		else if mat_`vlab'_`g'[4,2] < 0.1 {
			local stars "*"
		}
		else {
			local stars ""
		...
		local coef_`vlab'_`g' : display %4.3f mat_`vlab'_`g'[1,2] "`stars'"
		mat drop mat_`vlab'_`g'
		}
...
*- Figure ECE (no 8th)
	coefplot ///
			...
			grid(none) ///
			bycoefs 
			capture qui graph export "$FIGURES\TWFE\ece_2_4_T`treatment_type'_S`subsample'_TEST.png", replace	
			capture qui graph export "$FIGURES\TWFE\ece_2_4_T`treatment_type'_S`subsample'_TEST.pdf", replace		

//local coef_ece_m_2p	= "-0.02***"		
//local coef_ece_m_4p	= "-0.02***"		
//local coef_ece_r_2p	= "-0.02***"		
//local coef_ece_r_4p	= "-0.02***"	
	
	coefplot ///
			(ece_m_2p_x_2_4_m, mcolor("${blue_1}") 	fcolor("${blue_1}%50")	lcolor("${blue_1}%50")	ciopts(color("${blue_1}"))) ///
			(ece_m_2p_x_2_4_f, mcolor("${red_1}") 	fcolor("${red_1}%50")	lcolor("${red_1}%50") 	ciopts(color("${red_1}"))) ///
			///(ece_m_2p_x_2_4_mf, mcolor("${green_1}") 	fcolor("${green_1}%50")	lcolor("${green_1}%50") 	ciopts(color("${green_1}"))) ///
			, ///
			bylabel("2nd grade (Mathematics)") ///
			|| ///
			(ece_r_2p_x_2_4_m, mcolor("${blue_1}") 	fcolor("${blue_1}%50")	lcolor("${blue_1}%50")	ciopts(color("${blue_1}"))) ///
			(ece_r_2p_x_2_4_f, mcolor("${red_1}") 	fcolor("${red_1}%50")	lcolor("${red_1}%50") 	ciopts(color("${red_1}"))) ///
			///(ece_r_2p_x_2_4_mf, mcolor("${green_1}") 	fcolor("${green_1}%50")	lcolor("${green_1}%50") 	ciopts(color("${green_1}"))) ///
			, ///
			bylabel("2nd grade (Reading)") ///
			|| ///
			(ece_m_4p_x_2_4_m, mcolor("${blue_1}") 	fcolor("${blue_1}%50")	lcolor("${blue_1}%50") 	ciopts(color("${blue_1}"))) ///
			(ece_m_4p_x_2_4_f, mcolor("${red_1}") 	fcolor("${red_1}%50")	lcolor("${red_1}%50") 	ciopts(color("${red_1}"))) ///
			///(ece_m_4p_x_2_4_mf, mcolor("${green_1}") 	fcolor("${green_1}%50")	lcolor("${green_1}%50") 	ciopts(color("${green_1}"))) ///
			, ///
			bylabel("4th grade (Mathematics)") ///
			|| ///
			(ece_r_4p_x_2_4_m, mcolor("${blue_1}") 	fcolor("${blue_1}%50")	lcolor("${blue_1}%50") 	ciopts(color("${blue_1}"))) ///
			(ece_r_4p_x_2_4_f, mcolor("${red_1}") 	fcolor("${red_1}%50")	lcolor("${red_1}%50") 	ciopts(color("${red_1}"))) ///
			///(ece_r_4p_x_2_4_mf, mcolor("${green_1}") 	fcolor("${green_1}%50")	lcolor("${green_1}%50") 	ciopts(color("${green_1}"))) ///
			, ///
			bylabel("4th grade (Reading)") ///
			|| ///
			, ///
			vertical ///
			recast(bar) barwidth(0.25) ///
			ciopts(recast(rcap)) citop ///
			xlabel(1 `""2nd grade" "(Mathematics)""' 2 `""2nd grade" "(Reading)""' 3 `""4th grade" "(Mathematics)""' 4 `""4th grade" "(Reading)""') ///	
			keep(treated_post) ///
			legend(order(1 "Boys" 3 "Girls"/* 5 "Boys-Girls"*/) col(3) pos(6)) ///
			ytitle("Standardized Exam relative to Only Children", size(medsmall) height(5)) ///							
			///xline(0, lcolor(gs12)) ///
			ylabel(-0.06(0.01)0.01) ///
			yline(0, lcolor(gs0) lpattern(dash)) ///
			grid(none) ///
			text(0.005 1 "Boys - Girls = `coef_ece_m_2p'", place(n) size(small)) ///
			text(0.005 2 "Δ = `coef_ece_r_2p'", place(n) size(small)) ///
			text(0.005 3 "Δ = `coef_ece_m_4p'", place(n) size(small)) ///
			text(0.005 4 "Δ = `coef_ece_r_4p'", place(n) size(small)) ///
			bycoefs 
			capture qui graph export "$FIGURES\TWFE\ece_2_4_gender_T`treatment_type'_S`subsample'.png", replace	
			capture qui graph export "$FIGURES\TWFE\ece_2_4_gender_T`treatment_type'_S`subsample'.pdf", replace	
			capture qui graph save "$FIGURES\TWFE\ece_2_4_gender_T`treatment_type'_S`subsample'.gph", replace	
			
/*			
gen sibs = fam_total_2>1

collapse score_math_4p score_com_4p  [iw=peso_m_4p], by(year male_siagie sibs)			
			
			
twoway 	(line score_math_4p year if sibs==0 & male_siagie==0, lcolor("${ek_red}%50")) 	///
		(line score_math_4p year if sibs==1 & male_siagie==0, lcolor("${ek_blue}%50"))	///
		(line score_math_4p year if sibs==0 & male_siagie==1, lcolor("${ek_red}")) 	///
		(line score_math_4p year if sibs==1 & male_siagie==1, lcolor("${ek_blue}"))	///
		, ///
		xtitle("Year") ///
