...
capture rm choice_heterogeneity_delta_swe.dta
capture rm choice_heterogeneity_delta_swe.sters
estimates clear

foreach char in earnings_degrees mean_gpa_enrolled retention_enrolled {
    xtile `char'_delta_p4 = `char'_delta if abs(cutoff_distance) <= ${bw_same_instprog_1}, nquantiles(4)
}
...
foreach type in instprog inst {
	foreach sib_app in all enrolls {
		...
		local est_settings 	= "bw(`bw') p(1) nofirst estsave(choice_heterogeneity_delta_swe)"
		...
		// Delta earnings
		est_reghdfe `y' if `if', ///
			iv(1.enrolled#i.earnings_degrees_delta_p4 = 1.above_cutoff#i.earnings_degrees_delta_p4) ///
			fe(i.birthyear_sib i.id_cutoff i.id_cutoff_k) ///
			`est_settings' m("`type'_`sib_app'_1")

		// Delta peer quality
		est_reghdfe `y' if `if', ///
			iv(1.enrolled#i.mean_gpa_enrolled_delta_p4 = 1.above_cutoff#i.mean_gpa_enrolled_delta_p4) ///
			fe(i.birthyear_sib i.id_cutoff i.id_cutoff_k) ///
			`est_settings' m("`type'_`sib_app'_2")

		// Delta retention
		est_reghdfe `y' if `if', ///
			iv(1.enrolled#i.retention_enrolled_delta_p4 = 1.above_cutoff#i.retention_enrolled_delta_p4) ///
			fe(i.birthyear_sib i.id_cutoff i.id_cutoff_k) ///
...
// Table output
estread using choice_heterogeneity_delta_swe
