...
foreach level in "colmaj" "college" {
		...
		foreach choice in "apply" "enroll" {
			...
			foreach var2 of varlist same_gender age_diff5 {

				local contador = `contador' + 1

					if `contador' == 1 local text gend
					if `contador' == 2 local text age

					tab `var2', gen(c_)

			    gen aux_1 = c_2
					gen interaction_1 = above_cutoff*c_2
					gen enrolls_interaction_1 = enrolls_old*c_2
					...
					#delimit cr
					estadd ysumm
					estimates store ols_`level'_`text'
					...
					estimates store iv_`level'_`text'
					...
					*** Save results
					#delimit;
					estout ols_`level'_gend ols_`level'_age
					using "ols_`choice'_`level'_closeness_p1.tex",
					...
					#delimit;
					estout iv_`level'_gend iv_`level'_age
					using "iv_`choice'_`level'_closeness_p1.tex",
					cells(b(star fmt(%9.5f)) se(par fmt(%9.5f)))
