...
*** Target
foreach var of varlist qts1 {
  foreach level in "colmaj" "college" {
	   foreach choice in   "apply" "enroll" {

		if "`level'" == "colmaj" 	  local if "wl_students > 0 & oldest == 1  & codigo_demre_next !=. & ts1 !=."
		if "`level'" == "college" 	local if "wl_students > 0 & college_sample == 1 & oldest == 1 & codigo_demre_next !=. & ts1 !=."
