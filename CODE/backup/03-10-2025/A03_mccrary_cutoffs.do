...
program define mccrary_cutoff
	...
	*- See sample type (noz = not at zero/cutoff)
	...
	keep codigo_modular semester type_admission facultad major_c1_code id_cutoff cutoff_raw score_relative not_at_cutoff
	if $test == 1 & "`cell'" == "department" drop major_c1_code
	...
	*- To speed up, we save directly those with no cutoff (and hence, no mccrary) instead of including in the loop.
	...
	foreach l of local levels {
		...
		keep codigo_modular semester type_admission facultad major_c1_code id_cutoff mccrary*
