...
program define score_cutoff
	...
	log using "$LOGS/score_cutoff_department_`cell'.log", text replace
	...
	*Now create a loop to have all the cutoffs from all the loteries 
	...
	if "`cell'" == "department"  {
		keep 	codigo_modular semester type_admission facultad source ///
				id_cutoff_department 	rank_score_raw_department 	score_raw score_std_department 	admitted 
		rename 	(id_cutoff_department 	rank_score_raw_department 	score_raw score_std_department 	admitted) ///
	...
	if "`cell'" == "major"  {
		keep 	codigo_modular semester type_admission facultad major_c1_code source ///
				id_cutoff_major rank_score_raw_major score_raw score_std_major admitted 
		rename 	(id_cutoff_major 	rank_score_raw_major 	score_raw score_std_major 	admitted) ///
				(id_cutoff 			rank_score_raw 			score_raw score_std 		admitted)
		}
	
	*- We exclude cutoffs with no score
	bys id_cutoff: egen has_score = count(rank_score_raw)
	drop if has_score==0
	drop has_score
	
	*- We only look within relevant scores (Just before first admitted and just after last non-admitted)
	...
	bys id_cutoff_major	(rank_score_raw_major): replace upper_bound = last_0 	if last_0==rank_score_raw_major[_N]

	gen relevant_scores = (cutoff_rank_major>=lower_bound & cutoff_rank_major<=upper_bound) if cutoff_rank_major!=.
	...
	*- Statistics only requiring max R2
	...
	if $test == 0  save "$TEMP\applied_cutoffs_`cell'.dta", replace emptyok
	if $test == 1  save "$TEMP\applied_cutoffs_`cell'_TEST.dta", replace emptyok
	...
	foreach l of local levels {
		...
		sum rank_score_raw	
		levelsof rank_score_raw if rank_score_raw != `r(min)' & rank_score_raw != `r(max)', c
	...
	*If the obs are more than zero, means it was estimated so we keep it and store the corresponding lottery number
		...
		keep if _n==1
		if $test == 0 {
			append using  "$TEMP\applied_cutoffs_`cell'.dta"
			save  "$TEMP\applied_cutoffs_`cell'.dta", replace		
				
		if mod(`l',60)==10 {
			use "$TEMP\applied_cutoffs_`cell'.dta", clear
			save "$TEMP\applied_cutoffs_`cell'_TEMP.dta", replace //every 60 starting at 5.	
			}
		}
		
		if $test == 1 {
			append using  "$TEMP\applied_cutoffs_`cell'_TEST.dta"
			save  "$TEMP\applied_cutoffs_`cell'_TEST.dta", replace		
				
		if mod(`l',60)==10 {
			use "$TEMP\applied_cutoffs_`cell'_TEST.dta", clear
			save "$TEMP\applied_cutoffs_`cell'_TEMP_TEST.dta", replace //every 60 starting at 5.	
			}
		}		
	...
	if $test == 1 use "$TEMP/applied_cutoffs_`cell'_TEST.dta", clear
	...
	if "`cell'" == "major" 			rename (codigo_modular_`cell' semester_`cell' type_admission_`cell' facultad_`cell' major_c1_code_`cell' 	source_`cell') (codigo_modular semester type_admission facultad major_c1_code 	source)
	if "`cell'" == "department" 	rename (codigo_modular_`cell' semester_`cell' type_admission_`cell' facultad_`cell'  						source_`cell') (codigo_modular semester type_admission facultad  				source)
	
	compress

		
	if $test == 0 save "$TEMP/applied_cutoffs_`cell'.dta", replace
	if $test == 1 save "$TEMP/applied_cutoffs_`cell'_TEST.dta", replace
