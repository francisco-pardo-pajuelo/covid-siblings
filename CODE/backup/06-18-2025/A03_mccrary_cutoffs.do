...
mccrary_cutoff		noz deprt
mccrary_cutoff 		all deprt

mccrary_cutoff 		noz	major_full
mccrary_cutoff 		all major_full

mccrary_cutoff		noz deprt_full
mccrary_cutoff 		all deprt_full
...
capture program drop setup_A03
program define setup_A03
...
program define mccrary_all
	...
	*- Admin (all/public/private)
	...
	keep id_cutoff cutoff_raw score_relative public not_at_cutoff admin codigo_modular year
...
program define histograms_university
	...
	*- See sample type (noz = not at zero/cutoff)
	...
	keep codigo_modular score_relative not_at_cutoff admitted ABOVE
	
	compress 
	
	levelsof codigo_modular, local(levels)

	foreach l of local levels {
		di `l'
		local N = r(N)
		
		capture binsreg admitted score_relative if abs(score_relative)<$window & not_at_cutoff==1 & codigo_modular == `l',  savedata($TEMP\binsreg_`l') replace  by(ABOVE) bycolors(stc1 stc1) bysymbols(O O) bylpatterns(solid solid) legend(off) //nbins(50) 
		...
		if _rc!=0 | fileexists("$TEMP\binsreg_`l'.dta")==0 capture binsreg admitted score_relative if abs(score_relative)<$window & not_at_cutoff==1 & codigo_modular == `l',  savedata($TEMP\binsreg_`l') replace  //by(ABOVE) bycolors(stc1 stc1) bysymbols(O O) bylpatterns(solid solid) legend(off) //nbins(50) 		
		
		//It failed a couple of times 
		forvalues nbins = 100(-10)10 {
		if _rc!=0 | fileexists("$TEMP\binsreg_`l'.dta")==0 capture binsreg admitted score_relative if abs(score_relative)<$window & not_at_cutoff==1 & codigo_modular == `l',  savedata($TEMP\binsreg_`l') replace nbins(`nbins')
		}
		
		preserve
			append using "$TEMP\binsreg_`l'"
			
			twoway 	///
					(histogram score_relative if codigo_modular == `l' & abs(score_relative)<$window & not_at_cutoff==1, color(green%30) ytitle("Histogram Density", axis(1))) ///
			...
			graph export 	"$FIGURES\TEMP\histogram_score_`l'_`type'_`cell'.png", replace
			
			erase "$TEMP\binsreg_`l'.dta"
		restore
...
program define mccrary_cutoff
	...
	*- See sample type (noz = not at zero/cutoff)
	...
	if $test == 1 {
		bys id_cutoff: egen u = max(cond(_n==1,runiform(),.))
		egen u_rank = group(u)
		keep if u_rank<100
	}
	 
	 
	save "$TEMP\mccrary_test_`cell'", replace

	*- To speed up, we save directly those with no cutoff (and hence, no mccrary) instead of including in the loop.
	keep if cutoff_raw==.
	bys id_cutoff: keep if _n==1
	drop cutoff_raw score_relative not_at_cutoff
	gen double mccrary_pv_def = .
	gen double mccrary_pv_biasi = .
	gen double mccrary_test = .
		
	save "$TEMP/mccrary_cutoffs_`type'_`cell'${data}.dta", replace emptyok
	 

	use "$TEMP\mccrary_test_`cell'", clear

	//Exclude those cells with no estimated cutoff score
	drop if cutoff_raw==.
	
	//Exclude those observations at cutoff (if type = "noz")
	...
	foreach l of local levels {
		...
		preserve
			...
			use if id_cutoff == `l' using "$TEMP\mccrary_test_`cell'", clear
			...
			append using  "$TEMP/mccrary_cutoffs_`type'_`cell'${data}.dta"
			save  "$TEMP/mccrary_cutoffs_`type'_`cell'${data}.dta", replace			
			
			compress
			
			if mod(`l',60)==10 save "$TEMP/mccrary_cutoffs_`type'_`cell'_TEMP${data}.dta", replace //every 60 starting at 5.	
		restore
	}
	
	
	use  "$TEMP/mccrary_cutoffs_`type'_`cell'${data}.dta", clear
	
	merge 1:1 id_cutoff using 	"$TEMP\id_cutoff_info_`cell'", keep(match) nogen

	sort id_cutoff
	
	isvar id_cutoff codigo_modular semester facultad major_c1_code type_admission mccrary_*
	...
	rename (codigo_modular_`type'_`cell' semester_`type'_`cell' type_admission_`type'_`cell' facultad_`type'_`cell') (codigo_modular semester type_admission facultad)
	capture rename major_c1_code_`type'_`cell' major_c1_code
	
	compress
	
	save  "$TEMP/mccrary_cutoffs_`type'_`cell'${data}.dta", replace
	
	erase "$TEMP\mccrary_test_`cell'.dta"
...
program define figure_example
	...
	keep if codigo_modular == 160000001 | codigo_modular == 160000004
	...
	*- See sample type (noz = not at zero/cutoff)
	...
	keep codigo_modular score_relative not_at_cutoff admitted ABOVE	
		...
		binsreg admitted score_relative if abs(score_relative)<$window & not_at_cutoff==1 & codigo_modular == `l',  savedata($TEMP\binsreg_example_`l') replace  by(ABOVE) bycolors(stc1 stc1) bysymbols(O O) bylpatterns(solid solid) legend(off) //nbins(50) 

		preserve
			append using "$TEMP\binsreg_example_`l'"
			...
			graph export 	"$FIGURES\TEMP\histogram_score_`l'_`type'_`cell'_example1.png", replace
			...
			graph export 	"$FIGURES\TEMP\histogram_score_`l'_`type'_`cell'_example2.png", replace
			...
			graph export 	"$FIGURES\TEMP\histogram_score_`l'_`type'_`cell'_example3.png", replace
						
			erase "$TEMP\binsreg_example_`l'.dta"
		restore
	//}	
