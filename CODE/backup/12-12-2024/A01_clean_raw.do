...
program define setup
	...
	label define public 0 "Private" 1 "Public", replace
	label define level 1 "Pre-school" 2 "Primary" 3 "Secondary" , replace
end
...
foreach y in "2014" "2015" "2016" "2017" "2018" "2019" "2020" "2021" "2022" "2023" {
	...
	*- Public school
	gen public = 1 if gestion_siagie=="PUBLICO" 
...
*- Student 2S
	...
	if fileexists("$TEMP\ece_family_2016_2s.dta") & ${excel}!=1  use  "$TEMP\ece_family_2016_2s", clear
		...
		merge 1:1 id_estudiante using `ece_2s_2016_f2', keep(master match) //
		drop _m
	...
	**# THERE IS AN EM FOR WRITING THIS YEAR
	...
	if fileexists("$TEMP\em_family_2022_2s.dta") & ${excel}!=1  use  "$TEMP\em_family_2022_2s", clear
		...
		merge 1:1 id_estudiante using `em_2s_2022_d2', keep(master match) //
		drop _m	
	...
	if fileexists("$TEMP\em_family_2023_2s.dta") & ${excel}!=1  use  "$TEMP\em_family_2023_2s", clear
		...
		merge 1:1 id_estudiante using `em_2s_2023_d3', keep(master match) //
		drop _m	
...
foreach y in "2014" "2015" "2016" "2017" "2018" "2019" "2020" "2021" "2022" "2023" {
	...
	*- In absence of DOB we rank them based on starting year
	...
	label var fam_order_4 "Sibling #order (father & mother & caretaker)"
	label var fam_total_4 "Total siblings (father & mother & caretaker)"
...
program define applied
	...
	isvar 			///
		...
		/*ID*/ codigo_modular id_tipo_institucion id_tipo_gestion id_anio id_codigo_facultad id_carrera_primera_opcion /*id_carrera_homologada_primera_op*/ /*id_estado_persona*/ id_periodo_postulacion periodo_postulacion id_periodo_matricula periodo_matricula	universidad facultad abreviatura_anio ///
		/*Char UNI*/ estatus_licenciamiento tipo_funcionamiento		///
		/*Char Indiv*/ 	fecha_nacimiento edad sexo	///
		/*applic info*/ puntaje_postulante codigo_carrera_inei_primera_opci carrera_primera_opcion nombre_carrera_inei_primera_opci codigo?_c2018  nombre1_c2018 codigo_carrera_inei_segunda_opci nombre_carrera_inei_segunda_opci 		///
...
*/	
	...
	*- Cutoff at exam level
	...
	egen id_cutoff_major = group(codigo_modular id_anio id_periodo_postulacion major_c1_code source)
	...
	*- Label
	...
	label values admitted yes_no
		...
		isvar 			///
			...
			/*applic info*/ major_c1_cat* major_c1_code major_c1_name major_c1_inei_code   /*id_major* name_major* carrera_primera_opcion*/ score_raw score_std* rank_score_raw*	first_sem_application one_application* source issue 	///
	...
	if ${test}==0 {
		...
		drop u
		compress		
		save "$TEMP\applied_test", replace				
	}
...
program define enrolled
	...
	*- Average Grade
	...
	isvar 			///
		/*Match ID*/ id_per_umc id_persona_rec ///
		/*ID*/ codigo_modular  id_anio id_codigo_facultad id_carrera_primera_opcion /*id_carrera_homologada_primera_op*/ id_estado_persona id_periodo_postulacion  id_periodo_matricula semester	universidad	year  ///
		/*Char UNI*/ licensed	public academic university	region ///
		/*Char Indiv*/ 	dob age male ///
		/*applic info*/ carrera_primera_opcion		///
		/*admitt info*/ es_ingresante carrera_ingreso ///
		/*enroll info*/ score_std_uni major_inei_name facultad major_code major_inei_code major_cat*
		local all_vars = r(varlist)
		ds `all_vars', not
		keep `all_vars'
		order `all_vars'
	...
	if ${test}==0 {
		...
		drop u
		compress		
		save "$TEMP\enrolled_TEST", replace
	}
...
*- Graduation rate (at university) - Needs duration and graduation database.

*- Dropout rate (at university) 
...
use "$TEMP\applied", clear
		...
		isvar 			///
			/*Match ID*/ id_per_umc   ///
			/*ID*/ year id_cutoff_* universidad facultad semester  ///
			/*Char UNI*/  public ///
			/*Char Indiv*/ 	dob age male	///
	...
	*- Application outcomes:
		...
		*- Database of # of applications total
		preserve
			...
			save "$TEMP\total_applications_student_semester", replace
		restore
		
		*Total applications overall and average score in first take
		preserve
			keep id_per_umc year semester universidad score_std_department score_std_major
			...
			save "$TEMP\overall_applications_student", replace
		restore
		...
		*- Filter incoming students per major-semester
		preserve
			keep if admitted==1
			keep id_per_umc universidad /*facultad*/ major_admit_inei_code year semester
			bys id_per_umc universidad /*facultad*/ major_admit_inei_code year semester: keep if _n==_N
		...
		*- Database per student-university-major-semester
		bys id_per_umc public universidad /*facultad*/ major_c1_inei_code year semester: keep if _n==_N
		keep id_per_umc public universidad /*facultad*/ major_c1_inei_code year semester dob
		...
		*- Database per student-university-semester
		...
		bys id_per_umc universidad semester: keep if _n==_N
		keep id_per_umc universidad year semester
		...
		*- Database per student-university-major
		use "$TEMP\applied_students_university_major_semester", clear
		bys id_per_umc universidad /*facultad*/ major_c1_inei_code (year semester): keep if _n==_N
		keep id_per_umc year universidad /*facultad*/ major_c1_inei_code
		save "$TEMP\applied_students_university_major", replace		
	
		*- Database per student-university
		use "$TEMP\applied_students_university_major_semester", clear
		bys id_per_umc universidad (year semester): keep if _n==_N
		keep id_per_umc year universidad
		...
		*- Database per student
		use "$TEMP\applied_students_university_major_semester", clear
		bys id_per_umc (year semester): keep if _n==_N
		keep id_per_umc year dob semester
...
use "$TEMP\enrolled", clear
		...
		isvar 			///
			/*Match ID*/ id_per_umc   ///
			/*ID*/ year universidad facultad major_code major_inei_code semester major_cat ///
			/*Char UNI*/  public ///
			/*Char Indiv*/ 	dob age male	///
	...
	*- Enrollment outcomes:
		...
		*-Average school quality:
		//Incoming peer score
		preserve
			keep id_per_umc universidad /*facultad*/ major_inei_code year semester
			merge m:1 id_per_umc universidad /*facultad*/ major_inei_code year semester using "$TEMP\incoming_students", keep(master match)
			keep if _m==3
			drop _m
			...
			foreach ece_g in "2p" "4p" "2s" {
				foreach subj in "math" "com" "acad" {
					bys universidad /*facultad*/ major_inei_code year semester: egen avg_enr_score_`subj'_std_`ece_g' = mean(score_`subj'_std_`ece_g')
				}
			}
			drop score*
			drop m_*
			drop *4p //not enough naturally, since this were from 2016,2018, which would graduate in 2024 and apply 2025
			bys universidad /*facultad*/ major_inei_code year semester: keep if _n==1
		...
		*- Database per student-university-major-semester
		...
		bys id_per_umc public universidad /*facultad*/ major_inei_code year semester (neg_score_std_uni): keep if _n==1 //If more than one enrolled, keep the highest score
		keep id_per_umc public universidad /*facultad*/ major_inei_code year semester dob score_std_uni
		save "$TEMP\enrolled_students_university_major_semester", replace
		
		*- Database per student-university-semester
		use "$TEMP\enrolled_students_university_major_semester", clear
		gen neg_score_std_uni = -score_std_uni
		bys id_per_umc universidad semester (neg_score_std_uni): keep if _n==1 //If more than one enrolled, keep the highest score
		keep id_per_umc universidad year semester
		...
		*- Peer score of university first enrolled.
		use "$TEMP\enrolled_students_university_major_semester", clear
			keep id_per_umc universidad /*facultad*/ major_inei_code year semester score_std_uni
			merge m:1 id_per_umc universidad /*facultad*/ major_inei_code year semester using "$TEMP\incoming_students", keep(master match) 
			keep if _m==3
			drop _m
			//This are students in their first term after being admitted.
			merge m:1 universidad /*facultad*/ major_inei_code semester using "$TEMP\scores_incoming_university_major_semester", keep(master match) keepusing(avg_enr_score_*)
			gen neg_score_std_uni = -score_std_uni
		...
		*- Database per student-university-major
		use "$TEMP\enrolled_students_university_major_semester", clear
		bys id_per_umc universidad /*facultad*/ major_inei_code (year semester): keep if _n==_N
		keep id_per_umc year universidad /*facultad*/ major_inei_code
		...
		*- Database per student
		use "$TEMP\enrolled_students_university_major_semester", clear
		//We keep last university score and first university enrolled  ##
		bys id_per_umc (year semester): keep if _n==_N
		keep id_per_umc year dob score_std_uni
