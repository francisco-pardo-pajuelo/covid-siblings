...
setup
//student_final
//student_sibling_final 1
//student_sibling_final 2
//student_sibling_final 3
//student_sibling_final 4
...
program define student_final 
	...
	*- If max level is pre-school, then remove those observations as we will have no relevant outcomes:
	...
	*- Dropout
	...
	isvar /*Relevant vars*/ id_ie id_per_umc id_ie_??? exp_entry_year exp_graduating_year1 exp_graduating_year2 male_siagie region_siagie public_siagie urban_siagie carac_siagie grade year dropout* approved_???? approved_first_???? change_ie_???? pri_grad sec_grad year_graduate
	...
	*- Match Family info
	merge 1:1 id_per_umc using "$TEMP\id_siblings", keep(master match) keepusing(educ_caretaker educ_mother educ_father id_fam_* fam_order_* fam_total_*) 
	...
	*- Match EM exams
	...
	replace score_acad_std_2p 	= score_acad_std 	if score_acad_std==.
	...
	*- Peer score of first time enrolled 
	merge 1:1 id_per_umc using "$TEMP\enrolled_incoming_students_peer_scores", keepusing(avg_enr_score_*_std_??)  keep(master match) nogen
...
program define student_sibling_final 
	...
	*-- School characteristics
	merge m:1 id_per_umc using "$OUT\students", keepusing(id_ie region_siagie public_siagie urban_siagie carac_siagie id_ie_???) keep(master match) nogen
	...
	*-- Demographic
	merge m:1 id_per_umc using "$OUT\students", keepusing(male_siagie exp_graduating_year? socioec_index_* enrolled_lpred? last_year last_grade) keep(master match) nogen
	...
	*-- School progression
	merge m:1 id_per_umc using "$OUT\students", keepusing(dropout dropout_year dropout_grade dropout_ever approved_???? approved_first_???? change_ie_???? pri_grad sec_grad year_graduate) keep(master match)  nogen
	...
	*-- Survey variables (Aspiration, ...)
	merge m:1 id_per_umc using "$OUT\students", keepusing(aspiration_?? higher_ed_?? std_belief_gender*_?? gender_miss_4p) keep(master match) nogen
	...
	*-- University (application)
	merge m:1 id_per_umc using "$OUT\students", keepusing(year_app applied year_app_public applied_public dob_app) keep(master match) nogen
	...
	*- We recover the IDs (id_per_umc) of each sibling based on sibling order
	...
	bys aux_id_per_umc: replace fam_order_${fam_type} = fam_order_${fam_type} + _n //next 5 siblings
...
**# Bookmark #2 
	...
	*-- University (application)
	merge m:1 id_per_umc using "$OUT\students", keepusing(year_app applied year_app_public applied_public dob_app year_app_stem applied_stem year_app_nstem applied_nstem) keep(master match) nogen
	...
	*-- University (enrollment)
	merge m:1 id_per_umc using "$OUT\students", keepusing(dob_enr score_std_uni_enr avg_enr_score_*_std_??  year_enr_stem enrolled_stem year_enr_nstem enrolled_nstem) keep(master match) nogen
...
*/
	...
	*- Age differenec
	...
	save "$OUT\students_siblings_${fam_type}", replace
...
program define application_averages
	...
	keep id_per_umc semester university id_cutoff_department id_cutoff_major
	...
	*-- Exam
	...
	foreach v of var score_*_std_?? {
		...
		bys university semester	: egen `v'_avg_uni 		= mean(`v')
	}
...
program define application_final 
	...
	*-- Demographic
	merge m:1 id_per_umc using "$OUT\students", keepusing(male_siagie exp_graduating_year? socioec_index_* *_lpred? last_year last_grade) keep(master match) nogen
	...
	*- Application outcomes:
		...
		*- Applied to private semester
		...
		//Ever			
		*- # of applications
		merge m:1 id_per_umc using "$TEMP\total_applications_student", keepusing(N_applications) keep(master match)  	
		assert _m==3 //based on same data so should be everyone
		rename (N_applications) (N_applications_foc)
		drop _m
...
*- Enrollment outcomes:
	...
	clonevar major_inei_code = major_c1_inei_code
		...
		*- Enrolled in same uni-major 	semester
		merge m:1 id_per_umc universidad /*facultad*/ major_inei_code semester using "$TEMP\enrolled_students_university_major_semester", keepusing(year) keep(master match) 
		recode _m (1 = 0) (3 = 1)
		rename (_m year) (enroll_uni_major_sem_foc year_enr_uni_major_sem_foc)
	
		*- Enrolled in same uni 		semester
		merge m:1 id_per_umc universidad semester using "$TEMP\enrolled_students_university_semester", keepusing(year) keep(master match) 
		recode _m (1 = 0) (3 = 1)
		...
		*- Enrolled in same uni-major 	ever
		merge m:1 id_per_umc universidad /*facultad*/ major_inei_code  using "$TEMP\enrolled_students_university_major", keepusing(year) keep(master match) 
		recode _m (1 = 0) (3 = 1)
		rename (_m year) (enroll_uni_major_foc year_enr_uni_major_foc)		
		
		*- Enrolled in same uni 		ever
		merge m:1 id_per_umc universidad using "$TEMP\enrolled_students_university", keepusing(year) keep(master match) 
		recode _m (1 = 0) (3 = 1)
	...
	*- We create the matching variable for each sibling
	...
	bys id_application: replace fam_order_${fam_type} = fam_order_${fam_type} + _n //next 5 siblings
	sort id_application id_fam_${fam_type} fam_order_${fam_type}
	//drop if fam_order_${fam_type} > fam_total_${fam_type} //No need to look for siblings beyond the maximum size
	...
	*- Application outcomes
	merge m:1 id_per_umc universidad /*facultad*/ major_c1_inei_code using "$TEMP\applied_students_university_major", keepusing(year) keep(master match) 
	recode _m (1 = 0) (3 = 1)
	rename (_m year) (applied_uni_major_sib year_applied_uni_major_sib)
	
	merge m:1 id_per_umc universidad using "$TEMP\applied_students_university", keepusing(year) keep(master match) 
	recode _m (1 = 0) (3 = 1)
	...
	*- # of applications
	merge m:1 id_per_umc using "$TEMP\overall_applications_student", keepusing(N_applications score_std_major_avg score_std_department_avg) keep(master match)  	
	rename (N_applications score_std_major_avg score_std_department_avg) (N_applications_sib  score_std_major_avg_sib score_std_department_avg_sib) //No included in focal
	drop _m
...
//	bysort id_application (fam_order_4_sib): keep if (_n==1 & any_sib_match==1) | m_sibling_siagie==3 
	...
	*- Attach cutoff information (major)
		merge m:1 id_cutoff_major using  "$TEMP/applied_cutoffs_major.dta", keep(master match) keepusing(cutoff_rank_major cutoff_std_major)
...
isvar 			///
				...
				/*Sibling*/ ///
					...
					/*University*/ ///	
						/*Application */ 		year*app*sib app*sib semester_sib N_applications*sib score_std_*_avg_sib /// score_std_major_avg_sib score_std_department_avg_sib
				...
				/*INSTITUTION*/					universidad  public licensed academic	region	/// facultad university
				...
				*Destring those not IDs
				ds id_per_umc id_persona_rec, not
				local all_vars = r(varlist)
				destring `all_vars', replace
							
				compress
	
	save "$OUT/applied_outcomes_${fam_type}.dta", replace	
	
	merge m:1 id_per_umc using "$TEMP\students_TEST", keep(master match) keepusing(id_per_umc)
	bys id_fam_${fam_type}: egen is_test = max(_m)
	keep if _m==3
	drop _m is_test
	
	save  "$OUT/applied_outcomes_${fam_type}_TEST.dta", replace
	
	/*
	Reducing size tips:
	
	describe facultad universidad semester_sib
	describe facultad universidad semester_sib
