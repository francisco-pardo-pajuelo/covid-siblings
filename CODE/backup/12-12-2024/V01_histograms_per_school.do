...
capture program drop histo_uni
program define histo_uni
...
args type cell
	...
	preserve
		keep id_cutoff_major public universidad year
		bys id_cutoff_major public: keep if _n==1
		tempfile id_cutoff_major_database
		save `id_cutoff_major_database', replace
	restore
...
histogram score_relative if ${if_ex} & abs(score_relative)<1, bins(200) xline(0) // Looks odd when refining the bins.
...
global if_ex `"universidad == "UNIVERSIDAD NACIONAL DE PIURA" & semester=="2020-1" & strmatch(major_c1_name,"*DERECHO Y CIENCIAS*")"'
...
global if_ex = `"universidad == "PONTIFICIA UNIVERSIDAD CATÓLICA DEL PERÚ""'
di `"${if_ex}"'
sum mccrary_pv_def if ${if_ex}, de
...
global if_ex = `"universidad == "UNIVERSIDAD NACIONAL MAYOR DE SAN MARCOS""'
di `"${if_ex}"'
sum mccrary_pv_def if ${if_ex}, de
histogram score_relative if ${if_ex}, bins(100) 
//Good behavior public. Mcrary pvalues seem ~ to percentiles
