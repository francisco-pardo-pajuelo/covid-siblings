...
program define main_reg
	...
	*- 2. Application performance/effort
	...
	estimate_reg /*OUTCOME*/ score_std_major_avg_sib 	/*IV*/ enroll_uni_major_sem_foc 	/*label*/ app_sco_m_sib  	/*semesters*/ first /*bw*/ optimal
	preserve
...
// In terms of applications/enrollment	
	...
	file write table_tex	/// HEADER OPTIONS OF TABLE
					...
					/// HEADER OF TABLE
					"\toprule" _n ///
					"& Number of  		& Applies on   	& \multicolumn{3}{c}{Application Score}    \\" _n ///
					"\cline{2-3} \cline{4-6}" _n ///
					"& Applications  	& time 			& Any  & Same as sibling & Other public		\\" _n ///
					"& (1) 				& (2) 			& (3) 			& (4) 			& (5)	 \\" _n ///
					"\bottomrule" _n ///
					"&  &  &  &  & \\" _n 
	file close table_tex

	estout  iv_app_ot_sib  iv_N_apps_sib  iv_app_sco_m_sib  iv_app_sco_u_sib  iv_app_sco_puo_sib  ///
	using "$TABLES\\${t_tex}`suffix'.tex", ///
	append style(tex) ///
	cells(b(star fmt(%9.3f)) se(par fmt(%9.3f))) ///
	keep(${iv}) varlabels(${iv} "Older sibling enrolled") ///
	...
	estout  rf_app_ot_sib  rf_N_apps_sib  rf_app_sco_m_sib  rf_app_sco_u_sib rf_app_sco_puo_sib  ///
	using "$TABLES\\${t_tex}`suffix'.tex", ///
	append style(tex) ///
	cells(b(star fmt(%9.3f)) se(par fmt(%9.3f))) ///
	keep(ABOVE) varlabels(ABOVE "Older sibling above cutoff") ///
