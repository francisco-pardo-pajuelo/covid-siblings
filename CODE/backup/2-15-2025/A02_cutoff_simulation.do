...
program define score_cutoff
	...
	*Now create a loop to have all the cutoffs from all the loteries 
	...
	if "`cell'" == "department"  {
		keep id_cutoff_department 	rank_score_raw_department 	score_raw score_std_department 	admitted 
	...
	if "`cell'" == "major"  {
		keep id_cutoff_major rank_score_raw_major score_raw score_std_major admitted 
	...
	*If the obs are more than zero, means it was estimated so we keep it and store the corresponding lottery number
		...
		keep id_cutoff cutoff_rank cutoff_raw cutoff_std coeff p_val R2 N_below N_above has_cutoff /*without restrictions*/ cutoff_rank_all cutoff_raw_all cutoff_std_all coeff_all p_val_all R2_all N_below_all N_above_all has_cutoff_all
		keep if _n==1
		if $test == 0 {
	...
	sort id_cutoff
	order id_cutoff has_cutoff cutoff_raw cutoff_std cutoff_rank coeff p_val N_below N_above R2 /*without restrictions*/ has_cutoff_all cutoff_raw_all cutoff_std_all cutoff_rank_all coeff_all p_val_all N_below_all N_above_all R2_all  
