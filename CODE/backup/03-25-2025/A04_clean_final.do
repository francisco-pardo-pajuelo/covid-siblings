...
program define student_final 
	...
	foreach y in "2014" "2015" "2016" "2017" "2018" "2019" "2020" "2021" "2022" "2023" {
		append using "$TEMP\siagie_`y'", keep(id_ie id_per_umc level grade year male_siagie region_siagie public_siagie urban_siagie carac_siagie approved approved_first seccion_siagie)
	}
	...
	*- We only keep sections with 1 letter after some cleaning
	...
	encode seccion_siagie_temp2, gen(section_siagie)
	drop seccion_siagie*
	...
	*- Dropout
	...
	rename (year grade section_siagie) (last_year last_grade last_section)
...
//	bysort id_application (fam_order_4_sib): keep if (_n==1 & any_sib_match==1) | m_sibling_siagie==3 
	...
	*- Attach McCrary Tests (removing score=0) (major)
	foreach cutoff_level in "major" "deprt" "major_full" "deprt_full" {
		merge m:1 id_cutoff_`cutoff_level' using  "$TEMP/mccrary_cutoffs_noz_`cutoff_level'.dta", keep(master match)
		drop _m
	}	
