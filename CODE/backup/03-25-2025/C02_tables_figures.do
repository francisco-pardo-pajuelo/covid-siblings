...
program define first_stage_test
	...
	preserve 
		...
		*- Outcomes
		estimate_reg /*OUTCOME*/ applied_sib 				/*IV*/ enroll_uni_major_sem_foc 	/*label*/ test  /*sibling*/ older /*relative to application*/ ${main_rel_app} /*semesters*/ ${main_term} /*bw*/ ${main_bw} /*covs RD rob*/ semester_foc
	restore
...
program define first_stage
	...
	if "`results'" == "main" {
		...
		estimate_reg /*OUTCOME*/ `var' 				/*IV*/ none 						/*label*/ `var' 	/*sibling*/ oldest 				/*relative to application*/ all /*semesters*/ first /*bw*/ optimal	/*covs RD rob*/ ${main_covs_rdrob}		
		}
	...
	//Counterfactuals
	estimates restore rf_admitted
	local admitted = _b[ABOVE]
	estimates restore rf_enroll_uni_major_sem_foc
	local enroll_same_sem_uni = _b[ABOVE]
	estimates restore rf_enroll_uni_foc
	local enroll_uni = _b[ABOVE]
	estimates restore rf_enroll_private_foc
	local enroll_private = _b[ABOVE]
	estimates restore rf_enroll_foc
	local enroll_ever = _b[ABOVE]
	
	
	di `admitted' _n `enroll_same_sem_uni' _n `enroll_uni' _n `enroll_private' _n `enroll_ever'
	...
	if "`results'" == "R2" {
		...
		forvalues i = 1/3 {
			...
			estimate_reg /*OUTCOME*/ admitted 					/*IV*/ none 	/*label*/ fs_admitted_R2`i'  	/*sibling*/ oldest /*relative to application*/ all /*semesters*/ first /*bw*/ optimal		
			estimate_reg /*OUTCOME*/ enroll_uni_major_sem_foc 	/*IV*/ none 	/*label*/ fs_enrolled_R2`i'  	/*sibling*/ oldest /*relative to application*/ all /*semesters*/ first /*bw*/ optimal		
...
coefplot 	(rf_rd_1 rf_rd_2 rf_rd_3 rf_rd_4 rf_rd_5 rf_rd_6 rf_rd_7 rf_rd_8 rf_rd_9 rf_rd_10, mcolor(gs0) ciopts(color(/*gs0 gs0*/ gs0)) levels(/*99 95*/ 90)) ///
			(rf_rd_opt,mcolor(blue) ciopts(color(/*blue blue*/ blue)) levels(/*99 95*/ 90)), ///
				...
				legend(off)
	graph export 	"$FIGURES/eps/RD_sens_${outcome}_${siblings}_${rel_app}_${sem}_${bw_select}_${main_cell}.eps", replace	
	graph export 	"$FIGURES/png/RD_sens_${outcome}_${siblings}_${rel_app}_${sem}_${bw_select}_${main_cell}.png", replace	
	graph export 	"$FIGURES/pdf/RD_sens_${outcome}_${siblings}_${rel_app}_${sem}_${bw_select}_${main_cell}.pdf", replace		
...
args scale table_type heterog_type panel_A panel_B
	...
	file open  table_tex	using "$TABLES\\${t_tex}`heterog_type'_${main_cell}_PRES.tex", replace write
	...
	file open  table_tex	using "$TABLES\\${t_tex}`heterog_type'_${main_cell}_PRES.tex", append write
	file write table_tex	/// HEADER OPTIONS OF TABLE							
	_n "\bottomrule" _n ///
		"\end{tabular}" _n ///
		"}" _n ///
...
program define tables_input_main
	...
	if "`t_suffix'" == "DOC" 	local suffix = "_${main_cell}"
	if "`t_suffix'" == "PRES" 	local suffix = "_${main_cell}_PRES"
...
program define reg_test_new
	...
	global scores_1 		= "score_relative_1"
	global ABOVE_scores_1 	= "ABOVE_score_relative_1"
	global pot_outcomes "admitted applied_sib admitted_sib enroll_sib applied_uni_sib enrolled_uni_sib"
	global pot_ifs 		"fam_order_* first_sem* exp_grad* year_app_foc"
	...
	**## First exam?
	...
	local gap_before = 2
	local expected_grad_sib = "exp_graduating_year1_sib"
	global if_rel_app "(`expected_grad_sib'+`gap_after'>=year_app_foc)"
	...
	*-- Reduced form
		...
		preserve
			...
			reghdfe admitted ABOVE_`cell'  score_1_`cell' ABOVE_score_1_`cell' ///
					if ${if_final} ///
  ...
  5. |    21323       6792    1.349461   -.0560791          1          1    1.40554 |
	...
	*/
	...
	preserve
		...
		tab score_raw admitted if type_admission==1
		tab score_raw admitted if type_admission==2
		tab score_raw admitted if type_admission==8
		
		tab score_std_major admitted if type_admission==1
		tab score_std_major admitted if type_admission==2
		tab score_std_major admitted if type_admission==8
		
		tab score_std_major_full admitted if type_admission==1
		tab score_std_major_full admitted if type_admission==2
		tab score_std_major_full admitted if type_admission==8	
	restore
