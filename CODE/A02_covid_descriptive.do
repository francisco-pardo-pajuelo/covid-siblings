...
use "$TEMP\pre_reg_covid${covid_data}", clear
	...
	local shade_top2_com = 1.1
	local shade_bot2_com = .5
	local shade_top4_com = 0
	local shade_bot4_com = -0.3
	local shade_top8_com = 1	
	local shade_bot8_com = .5
	local arrow_2_com = .7
	local arrow_4_com = -.1
	local arrow_8_com = .8	
	local text_2_com = `arrow_2_com'-0.05
	local text_4_com = `arrow_4_com'-0.03	
	local text_8_com = `arrow_8_com'-0.04	
	
	local shade_top2_math = 1.1
	local shade_bot2_math = 0
	local shade_top4_math = 0
	local shade_bot4_math = -0.5
	local shade_top8_math = .8	
	local shade_bot8_math = .5
	local arrow_2_math = .5
	local arrow_4_math = -.2
	local arrow_8_math = .7
	local text_2_math = `arrow_2_math'-0.06
	local text_4_math = `arrow_4_math'-0.04	
	local text_8_math = `arrow_8_math'-0.04		
...
*/
	...
	foreach g in 2 4 8 {
		...
		preserve
			...
			// Create shade variables with continuous values in the range
			gen shade_top = `shade_top`g'_`subj'' 		if year >= 2019.5 & year <= 2021.5
			gen shade_bottom = `shade_bot`g'_`subj'' 	if year >= 2019.5 & year <= 2021.5

			// Fill in the gaps so you have continuous shading
			//replace shade_top = 0 if year > 2019.5 & year < 2021.5
			//replace shade_bottom = -0.5 if year > 2019.5 & year < 2021.5

			twoway 	(scatteri `shade_bot`g'_`subj'' 2019.5 `shade_top`g'_`subj'' 2019.5 `shade_top`g'_`subj'' 2021.5 `shade_bot`g'_`subj'' 2021.5 `shade_bot`g'_`subj'' 2019.5, ///
					 recast(area) color(gray%20) lwidth(none)) ///
					(line score_`subj'_${g${g}lab} year if sibs==0 & year<=2019, lcolor("${ek_red}")) 	///
					(line score_`subj'_${g${g}lab} year if sibs==1 & year<=2019, lcolor("${ek_blue}"))	///
					(line score_`subj'_${g${g}lab} year if sibs==0 & year>=2020, lcolor("${ek_red}")) 	///
					(line score_`subj'_${g${g}lab} year if sibs==1 & year>=2020, lcolor("${ek_blue}"))	///
					(pcarrowi `arrow_`g'_`subj'' 2019.5 `arrow_`g'_`subj'' 2021.5, ///
					 lcolor(black) barbsize(medium) mcolor(black)  ///
					 mlabcolor(black) mlabposition(6) mlabsize(medium)) ///			
					(pcarrowi `arrow_`g'_`subj'' 2021.5 `arrow_`g'_`subj'' 2019.5, ///
					 lcolor(black) barbsize(medium) mcolor(black) ///
					 mlabcolor(black) mlabposition(6) mlabsize(medium)) ///	
					, ///		
					text(`text_`g'_`subj'' 2020.5 "School Closures", placement(c)) ///
					xlabel(`xming`g''(1)`xmaxg`g'') ///
					ylabel(`shade_bot`g'_`subj''(.1)`shade_top`g'_`subj'') ///
					xline(2019.5 2021.5) ///
					xtitle("Year") ///
					ytitle("${g${g}tit} ${subj_title_`subj'}standardize exam score") ///
					legend(order(2 "Only Children" 3 "Children with Siblings") pos(6) col(2))
			...
			twoway 	(line pop year if sibs==0 & year<=2019, lcolor("${ek_red}")) 	///
					(line pop year if sibs==1 & year<=2019, lcolor("${ek_blue}"))	///
					(line pop year if sibs==0 & year>=2020, lcolor("${ek_red}")) 	///
					(line pop year if sibs==1 & year>=2020, lcolor("${ek_blue}"))	///
					, ///
					xlabel(`xming`g''(1)`xmaxg`g'') ///
					xtitle("Year") ///
			...
			twoway 	(line score year if sibs==0 & year<=2019, lcolor("${ek_red}")) 	///
					(line score year if sibs==1 & year<=2019, lcolor("${ek_blue}"))	///
					(line score year if sibs==0 & year>=2020, lcolor("${ek_red}")) 	///
					(line score year if sibs==1 & year>=2020, lcolor("${ek_blue}"))	///
					, ///
					xlabel(2014(1)2024) ///
					xtitle("Year") ///
...
args treatment_type subsample
	...
	*- But secondary changed to letters starting with cohort 2013 (7th grade in 2019)
	...
	use id_ie std_gpa_?* pass_math pass_read prim_on_time id_per_umc year grade treated min_socioec_index_ie_cat urban_siagie educ_cat_mother lives_with_mother lives_with_father t_born* fam_order_${fam_type} fam_total_${fam_type} ${x_all_vars} using "$TEMP\pre_reg_covid${covid_data}", clear
...
capture program drop raw_gpa_trends_main
program define raw_gpa_trends_main
...
args treatment_type subsample
	...
	*- But secondary changed to letters starting with cohort 2013 (7th grade in 2019)
	...
	local ytitle_std_gpa_m 		= "Standardized Mathematics GPA"		
	local ytitle_std_gpa_c 		= "Standardized Reading	GPA"			
	local ytitle_std_gpa_m_adj 	= "Standardized Mathematics GPA"		
	local ytitle_std_gpa_c_adj 	= "Standardized Reading GPA"		
	local ytitle_pass_math = "Passed Mathematics"		
	local ytitle_pass_read = "Passed Reading"	
	...
	*- By treatment type:
	...
	if "`treatment_type'"=="siblings" | "`treatment_type'"=="" {
		di "Continue, since treated is already defined as having siblings"
		local lab_control = "Only children"
		local lab_treated = "Children with siblings"
	}
	if "`treatment_type'"=="urban" {
		drop treated
	...
	local level = "elm"
			...
			//preserve 	
				...
				foreach v in /*"std_gpa_m" "std_gpa_c"*/ "std_gpa_m_adj" /*"std_gpa_c_adj" "pass_math" "pass_read" "prim_on_time"*/ {
					...
					local bot = -0.02 
					local arrow = 0.02
					local text = `arrow' - 0.002
					
					// Create shade variables with continuous values in the range
					gen shade_top = `top' if year >= 2019.5 & year <= 2021.5
					gen shade_bottom = `bot' if year >= 2019.5 & year <= 2021.5

					// Fill in the gaps so you have continuous shading
					//replace shade_top = 0.07 if year > 2019.5 & year < 2021.5
					//replace shade_bottom = -0.02 if year > 2019.5 & year < 2021.5					
								
						twoway 	(scatteri `bot' 2019.5 `top' 2019.5 `top' 2021.5 `bot' 2021.5 `bot' 2019.5, ///
								 recast(area) color(gray%20) lwidth(none)) ///
								(line `v' year if treated==0 & year>=2016, lcolor("${ek_red}")) /// 
								(line `v' year if treated==1 & year>=2016, lcolor("${ek_blue}")) ///
								(pcarrowi `arrow' 2019.5 `arrow' 2021.5, ///
								 lcolor(black) barbsize(medium) mcolor(black)  ///
								 mlabcolor(black) mlabposition(6) mlabsize(medium)) ///			
								(pcarrowi `arrow' 2021.5 `arrow' 2019.5, ///
								 lcolor(black) barbsize(medium) mcolor(black) ///
								 mlabcolor(black) mlabposition(6) mlabsize(medium)) ///	
								, ///		
								text(`text' 2020.5 "School Closures", placement(c)) ///
								xline(2019.5 2021.5, lcolor(gs8)) ///
								xlabel(2016 "16" 2017 "17" 2018 "18" 2019 "19" 2020 "20" 2021 "21" 2022 "22" 2023 "23" 2024 "24")  ///
								ytitle("`ytitle_`v''") ///
								xtitle("Year") ///
								///legend(off) ///
								legend(order(2 "`lab_control'" 3 "`lab_treated'") col(3) pos(6)) ///
								name(total_`v', replace)	
								
								
							if "${covid_data}" != "_TEST" {		
								capture qui graph export "$FIGURES\Descriptive\raw_shade_total_`level'_`v'_T`treatment_type'_S`subsample'_Size`size'${covid_data}.png", replace			
								capture qui graph export "$FIGURES\Descriptive\raw_shade_total_`level'_`v'_T`treatment_type'_S`subsample'_Size`size'${covid_data}.pdf", replace	
							}
							if "${covid_data}" == "_TEST" {		
								capture qui graph export "$FIGURES_TEMP\Descriptive\raw_shade_total_`level'_`v'_`treatment_type'_S`subsample'_Size`size'${covid_data}.png", replace			
								capture qui graph export "$FIGURES_TEMP\Descriptive\raw_shade_total_`level'_`v'_`treatment_type'_S`subsample'_Size`size'${covid_data}.pdf", replace	
							}
...
program define raw_histograms	
	...
	*- Aggregate: Elementary
	twoway 	(histogram math if inlist(grade,1,2,3,4,5,6)==1 & (inlist(year,2019)==1)  & fam_total_${fam_type}==1, lcolor("${ek_red}%40") fcolor("${ek_red}%40") discrete fraction) ///
			(histogram math if inlist(grade,1,2,3,4,5,6)==1 & (inlist(year,2019)==1)  & fam_total_${fam_type}>=2, lcolor("${ek_blue}%40") fcolor("${ek_blue}%40") discrete fraction) ///
	...
	twoway 	(histogram math if inlist(grade,1,2,3,4,5,6)==1 & (inlist(year,2020)==1)  & fam_total_${fam_type}==1, lcolor("${ek_red}%40") fcolor("${ek_red}%40") discrete fraction) ///
			(histogram math if inlist(grade,1,2,3,4,5,6)==1 & (inlist(year,2020)==1)  & fam_total_${fam_type}>=2, lcolor("${ek_blue}%40") fcolor("${ek_blue}%40") discrete fraction) ///
	...
	*- Aggregate: High school
	twoway 	(histogram math if inlist(grade,9,10)==1 & (inlist(year,2019)==1)  & fam_total_${fam_type}==1, lcolor("${ek_red}%40") fcolor("${ek_red}%40") discrete fraction) ///
			(histogram math if inlist(grade,9,10)==1 & (inlist(year,2019)==1)  & fam_total_${fam_type}>=2, lcolor("${ek_blue}%40") fcolor("${ek_blue}%40") discrete fraction) ///
			///(histogram math if grade==`g' & (inlist(year,2019)==1)  & fam_total_${fam_type}==3, lcolor(gs0) fcolor("${blue_2}%20") discrete fraction) ///
			///(histogram math if grade==`g' & (inlist(year,2019)==1)  & fam_total_${fam_type}==4, lcolor(gs0) fcolor("${blue_3}%20") discrete fraction) ///
			, ///
			/// legend(order(1 "No siblings" 2 "1 sibling" 3 "2 siblings" 4 "3 siblings") pos(6) col(4))
	...
	twoway 	(histogram math if inlist(grade,9,10)==1 & (inlist(year,2020)==1)  & fam_total_${fam_type}==1, lcolor("${ek_red}%40") fcolor("${ek_red}%40") discrete fraction) ///
			(histogram math if inlist(grade,9,10)==1 & (inlist(year,2020)==1)  & fam_total_${fam_type}>=2, lcolor("${ek_blue}%40") fcolor("${ek_blue}%40") discrete fraction) ///
			///(histogram math if grade==`g' & (inlist(year,2020)==1)  & fam_total_${fam_type}==3, lcolor(gs0) fcolor("${blue_2}%20") discrete fraction) ///
			///(histogram math if grade==`g' & (inlist(year,2020)==1)  & fam_total_${fam_type}==4, lcolor(gs0) fcolor("${blue_3}%20") discrete fraction) ///
			, ///
		legend(order(1 "No siblings" 2 "Siblings") pos(6) col(2)) ///
	...
	*- Grade by grade
	forvalues g = 1/7 {
		twoway 	(histogram math if grade==`g' & (inlist(year,2019)==1)  & fam_total_${fam_type}==1, lcolor("${ek_red}%40") fcolor("${ek_red}%40") discrete fraction) ///
				(histogram math if grade==`g' & (inlist(year,2019)==1)  & fam_total_${fam_type}>=2, lcolor("${ek_blue}%40") fcolor("${ek_blue}%40") discrete fraction) ///
				///(histogram math if grade==`g' & (inlist(year,2019)==1)  & fam_total_${fam_type}==3, lcolor(gs0) fcolor("${blue_2}%20") discrete fraction) ///
				///(histogram math if grade==`g' & (inlist(year,2019)==1)  & fam_total_${fam_type}==4, lcolor(gs0) fcolor("${blue_3}%20") discrete fraction) ///
				, ///
				/// legend(order(1 "No siblings" 2 "1 sibling" 3 "2 siblings" 4 "3 siblings") pos(6) col(4))
		...
		twoway 	(histogram math if grade==`g' & (inlist(year,2020)==1)  & fam_total_${fam_type}==1, lcolor("${ek_red}%40") fcolor("${ek_red}%40") discrete fraction) ///
				(histogram math if grade==`g' & (inlist(year,2020)==1)  & fam_total_${fam_type}>=2, lcolor("${ek_blue}%40") fcolor("${ek_blue}%40") discrete fraction) ///
				///(histogram math if grade==`g' & (inlist(year,2020)==1)  & fam_total_${fam_type}==3, lcolor(gs0) fcolor("${blue_2}%20") discrete fraction) ///
				///(histogram math if grade==`g' & (inlist(year,2020)==1)  & fam_total_${fam_type}==4, lcolor(gs0) fcolor("${blue_3}%20") discrete fraction) ///
				, ///
				legend(order(1 "No siblings" 2 "Siblings") pos(6) col(2)) ///
