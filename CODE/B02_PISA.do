...
program define import_data_2022
	...
	*- Parental Involvement
	egen parental_involvement = rmean(ST300Q??JA)
	VarStandardiz parental_involvement, newvar(std_parental_involvement)
	
	factor ST300Q??JA, pcf
	*rotate //Orthogonals does not seem to be working appropriately. Being these the relevant factors, we don't expect to get the orthogonal definitions. We consider the non rotated version.
	predict fac_parental_involvement	
...
program define descriptive
	...
	*- Distribution of SES for both sibling and only child sample
	...
	*- Parental Involvement
	...
	use "$TEMP\COVID\pisa_student_2022.dta", clear
	desc sibs ST230Q01JA 
	preserve
		twoway (kdensity std_parental_involvement if sibs==0 & CNT=="USA") ///
				(kdensity std_parental_involvement if sibs==1 & CNT=="USA") ///
				, ///
				xtitle("Parental Investment Index") ///
				legend(order(1 "1 Child" 2 "2+ children") col(2) pos(6)) 
				
		collapse PV1MATH std_parental_involvement, by(CNT CNTRYID sibs)

		
		twoway ///
				(scatter PV1MATH std_parental_involvement if sibs==0)  ///
				(scatter PV1MATH std_parental_involvement if sibs==1) ///
				, ///
				xtitle("Parental Involvement")
	restore
			
	collapse PV1MATH std_parental_involvement, by(CNT CNTRYID)
	scatter PV1MATH std_parental_involvement 
	
	merge m:1 CNT using "$TEMP\COVID\school_closure_country", keep(master match)
	
	scatter weeks_not_fully_open std_parental_involvement
