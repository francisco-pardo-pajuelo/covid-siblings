...
program define clean_data
	...
	*- Match ECE exams
	foreach g in "2p" "4p" /*"6p"*/ "2s" {
		...
		rename (/*year*/ score_math score_com score_math_std score_com_std score_acad_std) (/*year_`g'*/ score_math_`g' score_com_`g' score_math_std_`g' score_com_std_`g' score_acad_std_`g')
		rename (socioec_index socioec_index_cat) (socioec_index_`g' socioec_index_cat_`g')
		rename (label_m label_c) (label_m_`g' label_c_`g')
		rename (peso_m peso_c)	(peso_m_`g' peso_c_`g')
	}
	...
	*- Create Event Study vars	
	use id_per_umc id_ie grade year male_siagie urban_siagie educ_mother educ_father  lives_with_mother lives_with_father socioec* has_internet has_comp low_ses quiet_room approved* std* math comm score* satisf* fam_order_${fam_type} fam_total_${fam_type} base* year_2p peso* using "$TEMP\long_siagie_ece", clear
...
use "$TEMP\pre_reg_covid", clear
	...
	replace peso_m_4p = 1 if inlist(year,2016,2018)==1 & grade==4
	replace peso_c_4p = 1 if inlist(year,2016,2018)==1 & grade==4
	replace peso_m_2s = 1 if inlist(year,2015,2016,2018,2019)==1 & grade==8
	replace peso_c_2s = 1 if inlist(year,2015,2016,2018,2019)==1 & grade==8
...
*/
	...
	foreach g in 2 4 8 {
		...
		preserve
			...
			capture qui graph export "$FIGURES\COVID\raw_ece_`subj'_`g'.png", replace			
			capture qui graph export "$FIGURES\COVID\raw_ece_`subj'_`g'.pdf", replace		
			...
			capture qui graph export "$FIGURES\COVID\raw_ece_pop_`subj'_`g'.png", replace			
			capture qui graph export "$FIGURES\COVID\raw_ece_pop_`subj'_`g'.pdf", replace				
			...
			twoway 	(line score year if sibs==0 & year<=2019, lcolor("${red_1}")) 	///
					...
					legend(order(1 "Only Childs" 2 "Children with Siblings") pos(6) col(2))
			capture qui graph export "$FIGURES\COVID\raw_ece_rel_`subj'_`g'.png", replace			
			capture qui graph export "$FIGURES\COVID\raw_ece_rel_`subj'_`g'.pdf", replace		
		restore
		}
	}
	...
	restore
		...
		*- All individual plots	
		...
		graph combine   `v'_1 	///
			...
			///graph save "$FIGURES\COVID\raw_m.gph" , replace	
			///capture qui graph export "$FIGURES\COVID\raw_m.eps", replace	
			capture qui graph export "$FIGURES\COVID\raw_grades_`v'.png", replace			
			capture qui graph export "$FIGURES\COVID\raw_grades_`v'.pdf", replace	
		}
...
program define raw_histograms	
	...
	*- Aggregate: Elementary
	...
	capture qui graph export "$FIGURES\COVID\histogram_pre_elm.png", replace			
	capture qui graph export "$FIGURES\COVID\histogram_pre_elm.pdf", replace	
	...
	twoway 	(histogram math if inlist(grade,1,2,3,4,5,6)==1 & (inlist(year,2020)==1)  & fam_total_${fam_type}==1, lcolor("${red_1}%40") fcolor("${red_1}%40") discrete fraction) ///
		...
		xtitle("Mathematics grade") ///
		xlabel(1 "D" 2 "C" 3 "B" 4 "A") 
	capture qui graph export "$FIGURES\COVID\histogram_post_elm.png", replace			
	capture qui graph export "$FIGURES\COVID\histogram_post_elm.pdf", replace	
	...
	*- Aggregate: High school
	twoway 	(histogram math if inlist(grade,9,10)==1 & (inlist(year,2019)==1)  & fam_total_${fam_type}==1, lcolor("${red_1}%40") fcolor("${red_1}%40") discrete fraction) ///
			...
			legend(order(1 "No siblings" 2 "Siblings") pos(6) col(2)) ///
			xtitle("Mathematics grade") 
	capture qui graph export "$FIGURES\COVID\histogram_pre_9-10.png", replace			
	capture qui graph export "$FIGURES\COVID\histogram_pre_9-10.pdf", replace	
	...
	twoway 	(histogram math if inlist(grade,9,10)==1 & (inlist(year,2020)==1)  & fam_total_${fam_type}==1, lcolor("${red_1}%40") fcolor("${red_1}%40") discrete fraction) ///
		...
		legend(order(1 "No siblings" 2 "Siblings") pos(6) col(2)) ///
		xtitle("Mathematics grade")
	capture qui graph export "$FIGURES\COVID\histogram_post_9-10.png", replace			
	capture qui graph export "$FIGURES\COVID\histogram_post_9-10.pdf", replace		
	...
	*- Grade by grade
	forvalues g = 1/7 {
		twoway 	(histogram math if grade==`g' & (inlist(year,2019)==1)  & fam_total_${fam_type}==1, lcolor("${red_1}%40") fcolor("${red_1}%40") discrete fraction) ///
				...
				xtitle("Mathematics grade") ///
				xlabel(1 "D" 2 "C" 3 "B" 4 "A")
			capture qui graph export "$FIGURES\COVID\histogram_pre_grade`g'.png", replace			
			capture qui graph export "$FIGURES\COVID\histogram_pre_grade`g'.pdf", replace	
		...
		twoway 	(histogram math if grade==`g' & (inlist(year,2020)==1)  & fam_total_${fam_type}==1, lcolor("${red_1}%40") fcolor("${red_1}%40") discrete fraction) ///
				...
				xtitle("Mathematics grade") ///
				xlabel(1 "D" 2 "C" 3 "B" 4 "A") 
			capture qui graph export "$FIGURES\COVID\histogram_post_grade`g'.png", replace			
			capture qui graph export "$FIGURES\COVID\histogram_post_grade`g'.pdf", replace				
		}
	...
	*- TWFE Estimates
	...
	foreach level in "all" /*"elm" "sec"*/ {
		foreach v in "std_gpa_m" "std_gpa_c" "pass_math" "pass_read" /*"approved" "approved_first"*/ {
			...
			use std_gpa_? pass_math pass_read approved approved_first id_per_umc year_t_?? urban_siagie higher_ed_parent lives_with_mother lives_with_father has_internet has_comp low_ses quiet_room year grade treated post treated_post id_ie fam_order_${fam_type} fam_total_${fam_type} ${x} using "$TEMP\pre_reg_covid", clear
			...
			*/
			...
			///
			di as result "*******" _n as text "Urban areas" _n as result "*******"
			di as result "*******" _n as text "Rural areas" _n as result "*******"
			di as result "*******" _n as text "Male" _n as result "*******"
			di as result "*******" _n as text "Female" _n as result "*******"
			///
					...
					foreach res in "all" /*"alls"*/ /*"nint"*/ /*"ncom" "lses" "nqui"*/ { //all sample with data, No internet, no computer, low ses, no quiet room
						foreach v in /*"std_gpa_m" "std_gpa_c"*/ "pass_math" "pass_read"  {
							...
							*/
							...
							capture qui graph export "$FIGURES\COVID\covid_`v'_`area'_`lives_both_parents'_`hed'_`level'`young'_`res'`i'.png", replace	
							capture qui graph export "$FIGURES\COVID\covid_`v'_`area'_`lives_both_parents'_`hed'_`level'`young'_`res'`i'.pdf", replace								
						...
						forvalues i = 1/4 {
							...
							*/
							//graph save "$FIGURES\COVID\covid_gpa_`vlab'_`area'_`hed'_`level'`young'_`res'`i'.gph" , replace	
							//capture qui graph export "$FIGURES\COVID\covid_gpa_`vlab'_`area'_`hed'_`level'`young'_`res'`i'.eps", replace	
