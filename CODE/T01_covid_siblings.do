...
program define clean_data
	...
	keep id_per_umc year region_siagie public_siagie urban_siagie id_ie level grade male_siagie lives_with_mother lives_with_father approved approved_first std_gpa_m std_gpa_c math comm
	...
	*- Check how many years of data each school has (to use comparable samples)
	use id_ie year grade score_math_?? score_com_?? score_*_std_?? approved approved_first std_gpa_? math comm peso* using "$TEMP\long_siagie_ece", clear

		collapse  score_math_?? score_com_?? score_*_std_?? approved approved_first std_gpa_? math comm peso*, by(id_ie year)

		foreach v of var  score_math_?? score_com_?? score_*_std_?? approved approved_first std_gpa_? math comm peso* {
	...
	*- Create Event Study vars	
	use id_per_umc id_ie grade year male_siagie public_siagie urban_siagie educ_mother educ_father  lives_with_mother lives_with_father socioec* *has_internet *has_comp *low_ses *quiet_room approved* std* math comm score* satisf* fam_order_${fam_type} fam_total_${fam_type} base* year_2p peso* using "$TEMP\long_siagie_ece", clear
...
*/
	...
	foreach g in 2 4 8 {
		...
		preserve
			...
			capture qui graph export "$FIGURES\Descriptive\raw_ece_`subj'_`g'.png", replace			
			capture qui graph export "$FIGURES\Descriptive\raw_ece_`subj'_`g'.pdf", replace		
			...
			capture qui graph export "$FIGURES\Descriptive\raw_ece_pop_`subj'_`g'.png", replace			
			capture qui graph export "$FIGURES\Descriptive\raw_ece_pop_`subj'_`g'.pdf", replace				
			...
			twoway 	(line score year if sibs==0 & year<=2019, lcolor("${red_1}")) 	///
					...
					legend(order(1 "Only Childs" 2 "Children with Siblings") pos(6) col(2))
			capture qui graph export "$FIGURES\Descriptive\raw_ece_rel_`subj'_`g'.png", replace			
			capture qui graph export "$FIGURES\Descriptive\raw_ece_rel_`subj'_`g'.pdf", replace		
		restore
		}
	}
	...
	preserve 	
		...
		foreach v in "std_gpa_m" "std_gpa_c" "pass_math" "pass_read" {
			...
			twoway 		(rarea miny maxy areax if inrange(areax, 2019.5, 2021.5), color(gs15)) ///
						...
						xtitle("Year") ///
						///legend(off) ///
						legend(order(2 "Only childs" 4 "Children with siblings") col(3) pos(6)) ///
						name(total_`v', replace)	
						
						capture qui graph export "$FIGURES\Descriptive\raw_total_`v'.png", replace			
						capture qui graph export "$FIGURES\Descriptive\raw_total_`v'.pdf", replace	
	...
	preserve
		...
		*- All individual plots	
		...
		graph combine   `v'_2010 	///
			...
			capture qui graph export "$FIGURES\Descriptive\raw_cohorts_`v'.png", replace			
			capture qui graph export "$FIGURES\Descriptive\raw_cohorts_`v'.pdf", replace	
		}	
	restore
	
		collapse std_gpa_m std_gpa_c pass_math pass_read, by(year grade treated)
		...
		*- All individual plots	
		...
		graph combine   `v'_1 	///
			...
			///graph save "$FIGURES\raw_m.gph" , replace	
			///capture qui graph export "$FIGURES\raw_m.eps", replace	
			capture qui graph export "$FIGURES\Descriptive\raw_grades_`v'.png", replace			
			capture qui graph export "$FIGURES\Descriptive\raw_grades_`v'.pdf", replace	
		}
...
program define raw_histograms	
	...
	use std_gpa_? math comm pass_math pass_read id_per_umc year grade treated fam_order_${fam_type} fam_total_${fam_type} ${x} using "$TEMP\pre_reg_covid", clear
	...
	*- Aggregate: Elementary
	...
	capture qui graph export "$FIGURES\Descriptive\histogram_pre_elm.png", replace			
	capture qui graph export "$FIGURES\Descriptive\histogram_pre_elm.pdf", replace	
	...
	twoway 	(histogram math if inlist(grade,1,2,3,4,5,6)==1 & (inlist(year,2020)==1)  & fam_total_${fam_type}==1, lcolor("${red_1}%40") fcolor("${red_1}%40") discrete fraction) ///
		...
		xtitle("Mathematics grade") ///
		xlabel(1 "D" 2 "C" 3 "B" 4 "A") 
	capture qui graph export "$FIGURES\Descriptive\histogram_post_elm.png", replace			
	capture qui graph export "$FIGURES\Descriptive\histogram_post_elm.pdf", replace	
	...
	*- Aggregate: High school
	twoway 	(histogram math if inlist(grade,9,10)==1 & (inlist(year,2019)==1)  & fam_total_${fam_type}==1, lcolor("${red_1}%40") fcolor("${red_1}%40") discrete fraction) ///
			...
			legend(order(1 "No siblings" 2 "Siblings") pos(6) col(2)) ///
			xtitle("Mathematics grade") 
	capture qui graph export "$FIGURES\Descriptive\histogram_pre_9-10.png", replace			
	capture qui graph export "$FIGURES\Descriptive\histogram_pre_9-10.pdf", replace	
	...
	twoway 	(histogram math if inlist(grade,9,10)==1 & (inlist(year,2020)==1)  & fam_total_${fam_type}==1, lcolor("${red_1}%40") fcolor("${red_1}%40") discrete fraction) ///
		...
		legend(order(1 "No siblings" 2 "Siblings") pos(6) col(2)) ///
		xtitle("Mathematics grade")
	capture qui graph export "$FIGURES\Descriptive\histogram_post_9-10.png", replace			
	capture qui graph export "$FIGURES\Descriptive\histogram_post_9-10.pdf", replace		
	...
	*- Grade by grade
	forvalues g = 1/7 {
		twoway 	(histogram math if grade==`g' & (inlist(year,2019)==1)  & fam_total_${fam_type}==1, lcolor("${red_1}%40") fcolor("${red_1}%40") discrete fraction) ///
				...
				xtitle("Mathematics grade") ///
				xlabel(1 "D" 2 "C" 3 "B" 4 "A")
			capture qui graph export "$FIGURES\Descriptive\histogram_pre_grade`g'.png", replace			
			capture qui graph export "$FIGURES\Descriptive\histogram_pre_grade`g'.pdf", replace	
		...
		twoway 	(histogram math if grade==`g' & (inlist(year,2020)==1)  & fam_total_${fam_type}==1, lcolor("${red_1}%40") fcolor("${red_1}%40") discrete fraction) ///
				...
				xtitle("Mathematics grade") ///
				xlabel(1 "D" 2 "C" 3 "B" 4 "A") 
			capture qui graph export "$FIGURES\Descriptive\histogram_post_grade`g'.png", replace			
			capture qui graph export "$FIGURES\Descriptive\histogram_post_grade`g'.pdf", replace				
		}
	...
	foreach level in "all" /*"elm" "sec"*/ {
		foreach v in "std_gpa_m" "std_gpa_c" "pass_math" "pass_read" /*"approved" "approved_first"*/ {
			...
			*- Birth Order
			...
			//graph save "$FIGURES\covid_twfe_`level'_`vlab'_${max_sibs}.gph" , replace	
			//capture qui graph export "$FIGURES\covid_twfe_`level'_`vlab'_${max_sibs}.eps", replace	
			capture qui graph export "$FIGURES\TWFE\covid_twfe_B_`level'_`vlab'_${max_sibs}.png", replace	
			...
			if "`level'" == "all" {
				...
				//graph save "$FIGURES\covid_twfe_`level'_`vlab'_${max_sibs}.gph" , replace	
				//capture qui graph export "$FIGURES\covid_twfe_`level'_`vlab'_${max_sibs}.eps", replace	
				capture qui graph export "$FIGURES\TWFE\covid_twfe_cohort_`vlab'_${max_sibs}.png", replace	
				capture qui graph export "$FIGURES\TWFE\covid_twfe_cohort_`vlab'_${max_sibs}.pdf", replace					
			...
			capture qui graph export "$FIGURES\TWFE\covid_twfe_survey_`level'_`vlab'_${max_sibs}.png", replace	
			capture qui graph export "$FIGURES\TWFE\covid_twfe_survey_`level'_`vlab'_${max_sibs}.pdf", replace			
				...
				foreach hed in "all" /*"hed" "nhed"*/  { //none or at least one. # No change
					foreach res in "all" /*"alls"*/ /*"nint"*/ /*"ncom" "lses" "nqui"*/ { //all sample with data, No internet, no computer, low ses, no quiet room
						foreach v in "std_gpa_m" "std_gpa_c" "pass_math" "pass_read"  {
							...
							use `v' id_per_umc year_t_?? urban_siagie higher_ed_parent lives_with_mother lives_with_father *has_internet *has_comp *low_ses *quiet_room year grade treated post treated_post  id_ie fam_order_${fam_type} fam_total_${fam_type} ${x} using "$TEMP\pre_reg_covid", clear
							...
							*/
							...
							capture qui graph export "$FIGURES\Event Study\covid_`v'_`area'_`lives_both_parents'_`hed'_`level'`young'_`res'`i'.png", replace	
							capture qui graph export "$FIGURES\Event Study\covid_`v'_`area'_`lives_both_parents'_`hed'_`level'`young'_`res'`i'.pdf", replace								
		...
		foreach hed_parent in "no" "yes" "all" { //none or at least one
			...
			foreach res in "all" "alls" "nint" "ncom" "lses" "nqui" { //all sample with data, No internet, no computer, low ses, no quiet room
				...
				forvalues i = 1/4 {
					...
					capture qui graph export "$FIGURES\Event Study\covid_approved_`area'_`hed_parent'_`level'_`res'`i'.png", replace			
					capture qui graph export "$FIGURES\Event Study\covid_approved_`area'_`hed_parent'_`level'_`res'`i'.pdf", replace				
					}
				}	
			}
		estimates clear
		...
		foreach area in "all" "urb" "rur"  {
			foreach res in "all" /*"alls" "nint"*/ { 
				...
				*- Results by cohort
				...
				capture qui graph export "$FIGURES\Event Study\covid_cohort_`res'_`area'_`v'.png", replace				
				...
				capture qui graph export "$FIGURES\Event Study\covid_cohort_full_`res'_`area'_`v'.png", replace				
				...
				*/
				...
				capture qui graph export "$FIGURES\covid_grade_`res'_`area'_`v'.png", replace	
				...
				capture qui graph export "$FIGURES\covid_grade_full_`res'_`area'_`v'.png", replace					
				...
				capture qui graph export "$FIGURES\test_grade_`res'_`area'_`v'.png", replace	
				*/
			}
		}
	}	
