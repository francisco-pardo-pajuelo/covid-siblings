...
drop id_estudiante_2p source_2p merge_2p id_estudiante_4p source_4p merge_4p id_estudiante_2s source_2s merge_2s urban_2p merge_ece_2p merge_ece_4p merge_ece_2s merge_m_2p merge_m_4p merge_m_2s merge_ece_survey_2p merge_ece_survey_4p merge_ece_survey_2s
...
graph combine panel_A_STD_2p panel_B_STD_4p panel_C_STD_2s, col(1) xsize(17) ysize(20)
graph save "$FIGURES\COVID\covid_siblings_ece.gph" , replace	
capture qui graph export "$FIGURES\COVID\covid_siblings_ece.eps", replace	
capture qui graph export "$FIGURES\COVID\covid_siblings_ece.png", replace			
capture qui graph export "$FIGURES\COVID\covid_siblings_ece.pdf", replace	
	
	

*- GPA Overall
foreach v in 	/*GPA*/ 		"std_gpa_m" "std_gpa_c" ///
				/*APPROVED*/	"approved" "approved_first" {
	foreach level in "all" "elm" "sec" {						
		di "`v'"
		use `v' id_per_umc year_t_?? year grade treated id_ie fam_order_${fam_type} using "$TEMP\pre_reg_covid", clear
...
foreach level in "all" "elm" "sec" {
	coefplot 	std_gpa_m_`level' std_gpa_c_`level', ///
				omitted ///
				keep(year_t_??) msy(O) msize(1.5) vert recast(connected) ciopts(recast(rcap)) offset(0) ///
				...
				legend(pos(6) col(3)) ///
				name(panel_A_GPA_`level',replace)	
	graph save "$FIGURES\COVID\covid_ece_gpa_`level'.gph" , replace	
	capture qui graph export "$FIGURES\COVID\covid_ece_gpa_`level'.eps", replace	
	capture qui graph export "$FIGURES\COVID\covid_ece_gpa_`level'.png", replace			
	capture qui graph export "$FIGURES\COVID\covid_ece_gpa_`level'.pdf", replace	
	}

foreach level in "all" "elm" "sec" {
	coefplot 	approved_`level' approved_first_`level', ///
				omitted ///
				keep(year_t_??) msy(O) msize(1.5) vert recast(connected) ciopts(recast(rcap)) offset(0) ///
	...
	graph save "$FIGURES\COVID\covid_ece_approved_`level'.gph" , replace	
	capture qui graph export "$FIGURES\COVID\covid_ece_approved_`level'.eps", replace	
	capture qui graph export "$FIGURES\COVID\covid_ece_approved_`level'.png", replace			
	capture qui graph export "$FIGURES\COVID\covid_ece_approved_`level'.pdf", replace
	}


foreach level in "all" "elm" "sec" {
	graph combine panel_A_GPA_`level' panel_B_PASSED_`level', col(1) xsize(17) ysize(15)
	graph save "$FIGURES\COVID\covid_siblings_gpa_passed_`level'.gph" , replace	
	capture qui graph export "$FIGURES\COVID\covid_siblings_gpa_passed_`level'.eps", replace	
	capture qui graph export "$FIGURES\COVID\covid_siblings_gpa_passed_`level'.png", replace			
	capture qui graph export "$FIGURES\COVID\covid_siblings_gpa_passed_`level'.pdf", replace	
	}



/*
coefplot 	score_math_std_2p score_com_std_2p score_acad_std_2p, ///
			omitted ///
			keep(year_t_??) msy(O) msize(1.5) vert recast(connected) ciopts(recast(rcap)) offset(0) ///
			drop(year_t_b3) ///
			leg(order(1 "STD Math" 3 "STD Comm" 5 "STD Acad")) ///
			coeflabels(year_t_b6 = "2014" year_t_b5 = "2015" year_t_b4 = "2016" /*year_t_b3 = "2017"*/ year_t_b2 = "2018" year_t_o1 = "2019" /*year_t_a0 = "2020" year_t_a1 = "2021"*/ year_t_a2 = "2022" year_t_a3 = "2023") ///
			yline(0,  lcolor(gs10))  ///
			ytitle("Effect") ///
			subtitle("Panel A: Standardized Exams - 2th Grade") ///
			legend(pos(6) col(3)) ///
			name(panel_A_STD_2p,replace)	
graph save "$FIGURES\COVID\covid_ece_2p.gph" , replace	
capture qui graph export "$FIGURES\COVID\covid_ece_2p.eps", replace	
capture qui graph export "$FIGURES\COVID\covid_ece_2p.png", replace			
capture qui graph export "$FIGURES\COVID\covid_ece_2p.pdf", replace		

coefplot 	score_math_std_4p score_com_std_4p score_acad_std_4p, ///
			omitted ///
			keep(year_t_??) msy(O) msize(1.5) vert recast(connected) ciopts(recast(rcap)) offset(0) ///
			drop(year_t_b3) ///
			leg(order(1 "STD Math" 3 "STD Comm" 5 "STD Acad")) ///
			...
			yline(0,  lcolor(gs10))  ///
			ytitle("Effect") ///
			subtitle("Panel B: Standardized Exams - 4th Grade") ///
			legend(pos(6) col(3)) ///
			name(panel_B_STD_4p,replace)		
graph save "$FIGURES\COVID\covid_ece_4p.gph" , replace	
capture qui graph export "$FIGURES\COVID\covid_ece_4p.eps", replace	
capture qui graph export "$FIGURES\COVID\covid_ece_4p.png", replace			
capture qui graph export "$FIGURES\COVID\covid_ece_4p.pdf", replace	
	
coefplot 	score_math_std_2s score_com_std_2s score_acad_std_2s, ///
			omitted ///
			keep(year_t_??) msy(O) msize(1.5) vert recast(connected) ciopts(recast(rcap)) offset(0) ///
			drop(year_t_b3) ///
			leg(order(1 "STD Math" 3 "STD Comm" 5 "STD Acad")) ///
			...
			yline(0,  lcolor(gs10))  ///
			ytitle("Effect") ///
			subtitle("Panel C: Standardized Exams - 8th Grade") ///
			legend(pos(6) col(3)) ///
			name(panel_C_STD_2s,replace)		
graph save "$FIGURES\COVID\covid_ece_2s.gph" , replace	
capture qui graph export "$FIGURES\COVID\covid_ece_2s.eps", replace	
capture qui graph export "$FIGURES\COVID\covid_ece_2s.png", replace			
capture qui graph export "$FIGURES\COVID\covid_ece_2s.pdf", replace		
*/
...
keep if inlist(fam_total_2,1,2)==1

gen type_child = .
replace type_child = 1 if fam_total_2 == 1
replace type_child = 2 if fam_total_2 == 2 & fam_order_2 ==1
replace type_child = 3 if fam_total_2 == 2 & fam_order_2 ==2

preserve
	collapse score_*_std_2s, by(year_2s type_child)
	drop if year_2s==2020

	twoway 	(scatter score_math_std_2s year_2s if type_child==1) ///
			(scatter score_math_std_2s year_2s if type_child==2) ///
			(scatter score_math_std_2s year_2s if type_child==3) ///
			, ///
			legend(label(1 "Only Child") label(2 "1st child") label(3 "2nd/last child"))
		
restore


preserve
	collapse score_*_std_2s, by(year_2s fam_total_2)
	drop if year_2s==2020

	twoway 	(scatter score_math_std_2s year_2s if fam_total_2==1) ///
			(scatter score_math_std_2s year_2s if fam_total_2==2) ///
			, ///
			legend(label(1 "Only Child") label(2 "1 sibling"))
			

	twoway 	(scatter score_com_std_2s year_2s if fam_total_2==1) ///
			(scatter score_com_std_2s year_2s if fam_total_2==2) ///
			, ///
			legend(label(1 "Only Child") label(2 "1 sibling"))
		
		

	twoway 	(scatter score_acad_std_2s year_2s if fam_total_2==1) ///
			(scatter score_acad_std_2s year_2s if fam_total_2==2) ///
			, ///
			legend(label(1 "Only Child") label(2 "1 sibling"))
...
preserve
	collapse approved_???? approved_first_????, by(type_child)
	reshape long approved_ approved_first_, i(type_child) j(year)
	
	rename approved_ approved
	rename approved_first_ approved_first

	twoway 	(scatter approved year if type_child==1) ///
			(scatter approved year if type_child==2) ///
			(scatter approved year if type_child==3) ///
			, ///
			legend(label(1 "Only Child") label(2 "1st child") label(3 "2nd/last child"))
			
	twoway 	(scatter approved_first year if type_child==1) ///
			(scatter approved_first year if type_child==2) ///
			(scatter approved_first year if type_child==3) ///
			, ///
			legend(label(1 "Only Child") label(2 "1st child") label(3 "2nd/last child"))			
