...
capture program drop setup_ATU
program define setup_ATU
...
program define import_data
	...
	* Edit the import statement to reference the data file on your computer.;
	...
	t500107
	t509989
	 using "$IN\ATU\atussum_0324.dat", stringcols(1) clear ;
...
capture program drop analyze
program define analyze

	gen weight = tufnwgtp if tuyear!=2020
	replace weight = tu20fwgt if tuyear==2020 //use with care
	
	tab tuyear [iw=weight]

	
	foreach age_cat in "0-5" "6-11" "12-17" {
		preserve
		
			
				
				if "`age_cat'" == "0-5" 	keep if tryhhchild>=0 & tryhhchild<6
				if "`age_cat'" == "6-11" 	keep if tryhhchild>=6 & tryhhchild<12
				if "`age_cat'" == "12-17" 	keep if tryhhchild>=12 & tryhhchild<18
				
			egen t0301 = rowtotal(t0301??)
			egen t0302 = rowtotal(t0302??)	
			egen t0303 = rowtotal(t0303??)	
			
			egen t03 = rowtotal(t030?)
			
			collapse trchildnum t0301 t0302 t0303 t03 t0301?? t03020? [aw=weight], by(tuyear)
			
			gen covid = 0
			replace covid = 1 if inlist(tuyear,2020,2021)==1
			replace covid = 2 if inlist(tuyear,2022,2023,2024)==1
			
			list tuyear /*t0301?? t0302??*/ t030201 t030203 t03?? t03,  sepby(covid)
			
			twoway ///
				(line t030109 tuyear if tuyear>=2010) ///
				(line t030101 tuyear if tuyear>=2010) ///
				(line t030102 tuyear if tuyear>=2010) ///
				(line t030103 tuyear if tuyear>=2010) ///
				(line t030110 tuyear if tuyear>=2010) ///
				, /// 
				xlabel(2010(1)2024) ///
				xtitle("Year") ///
				ytitle("Minutes per day") ///
				legend(order(1 "Looking after hh children" 2 "Physical care for hh children" 3 "Reading to/with hh children" 4 "Playing with hh children, not sports" 5 "Attending hh children's events") pos(6) col(2))
			capture qui graph export "$FIGURES\Descriptive\ATU_A_`age_cat'.png", replace			
			capture qui graph export "$FIGURES\Descriptive\ATU_A_`age_cat'.pdf", replace			
			...
			twoway ///
				...
				xtitle("Year") ///
				ytitle("Minutes per day") ///
				legend(order(1 "Total" 2 "Caring For & Helping HH Children" 3 "Activities Related to HH Children's Education" 4 "Activities Related to HH Children's Health") pos(6) col(2))
			capture qui graph export "$FIGURES\Descriptive\ATU_summary_`age_cat'.png", replace			
