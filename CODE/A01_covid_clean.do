...
*/	
	...
	*- Variables only common to one or some grades:
	merge m:1 id_estudiante_2p  using "$TEMP\ece_family_2p", keep(master match) keepusing(age_cat_mother_?? lengua_materna_mother_?? hh_size_?? bedrooms_?? quiet_room_?? study_space_?? books_cat_?? years_prek_?? has_repeated_2nd_??) nogen
	...
	2s: edu_mother_`g' edu_father_`g' lengua_used_mother_`g'
	*/
		
	rename year_siagie year

	*- Match ECE exams and outcomes
	foreach g in "2p" "4p" "6p" "2s" {
	...
	foreach g in "2p" "4p" /*"6p"*/ "2s" {
		...
		isvar score_math score_com score_math_std score_com_std score_acad_std label_m label_c socioec_index socioec_index_cat urban spanish peso_m peso_c
		drop  `r(varlist)'
	}
	...
	*- Match ECE survey
	...
	drop id_estudiante_2p source_2p merge_2p id_estudiante_4p source_4p merge_4p id_estudiante_6p source_6p merge_6p id_estudiante_2s source_2s merge_2s merge_ece_?? merge_m_?? merge_ece_survey_* merge_*
	...
	*- Other control variables:
	...
	isvar 	/*ID*/ 			id_ie id_per_umc id_fam_${fam_type} fam_order_${fam_type} fam_total_${fam_type} fam_total12021_${fam_type} year level grade  ///
			/*GEO*/			section_siagie region_siagie public_siagie urban_siagie ///
			/*School*/		enrollment_grade_year enrollment_class_grade_year grade_size_average class_size_average quart_class_size quart_grade_size min_socioec_index_ie_cat ///
			/*Student*/		male_siagie dob_siagie exp_graduating_year1 exp_graduating_year2 year_entry_1st ///
			/*Outcomes*/	approved approved_first math comm std_gpa_m std_gpa_c std_gpa_m_adj std_gpa_c_adj ///
			/*Outcomes II*/	prim_on_time year_graduating_prim *parent_edu_inv* ///
			/*Adult*/ 		*caretaker* *mother* *father* ///
			/*Sibling*/		closest_age_gap_2 closest_age_gap_older_2 closest_age_gap_younger_2 closest_age_gap_samesex_2 closest_age_gap_male_2 closest_age_gap_female_2 total_siblings_2s ///
			/*Preg/Early*/	covid_preg_sib covid_0_2_sib covid_2_4_sib *born* ///
			/*Change IE*/	change_ie* ///
									/*ECE*/ ///
			/*year*/		year_2p 	year_4p 	year_2s ///
			/*Charact*/		urban_?? 						///
			/*Baseline*/	base*2p 	base*4p 	base*6p base*2s ///
			/*Scores*/		score*2p 	score*4p 	score*6p score*2s ///
			/*Labels*/		label*2p 	label*4p 	label*2s satisf*2p satisf*4p satisf*2s ///
			/*Weights*/		peso*2p 	peso*4p 	peso*2s ///
			/*Aspiration*/	aspiration_fam_2p aspiration_fam_4p aspiration_fam_6p aspiration_stu_6p aspiration_stu_2s ///
			/*SES*/			socioec_index_?? socioec_index_cat_?? ///
			/*Other*/		lives_with_mom_2s lives_with_dad_2s lives_with_grandparents_2s lives_with_siblings_2s lives_with_uncle_aunt_2s lives_with_other_fam_2s lives_with_other_non_fam_2s
	...
	*- Create Event Study vars	
	use 	///
		...
		fam_order_${fam_type} fam_total_${fam_type} fam_total12021_${fam_type} ///
		...
		*- Parent's education
		label define educ_cat 1 "Did not complete secondary education" 2 "Completed secondary education" 3 "Some level of higher education"
		label define survey_educ_2p_4p 1 "No studies" 2 "Incomplete Primary" 3 "Complete Primary" 4 "Incomplete Secondary" 5 "Complete Secondary" 6 "Incomplete Ocupational Education" 7 "Complete Ocupational Education" 8 "Incomplete 2-year college" 9 "Complete 2-year college" 10 "Incomplete 5-year college" 11 "Complete 5-year college" 12 "Post-Graduate"
		label define survey_educ_2s 1 "No studies" 2 "Incomplete Primary" 3 "Complete Primary" 4 "Incomplete Secondary" 5 "Complete Secondary" 6 "Incomplete 2-year college" 7 "Complete 2-year college" 8 "Incomplete 5-year college" 9 "Complete 5-year college" 10 "Post-Graduate"
		
		//lab li edu // educ_mother and educ_father  ->  1 None 2 Primary Incomplete 3 Primary Complete 4 Secondary Incomplete   5 Secondary Complete  6 Higher Incomplete 7 Higher Complete 8 Post-grad
		*-- Higher Ed parents
		...
		*-- Father's education (SIAGIE)
		...
		label values educ_cat_father educ_cat
 		
		*-- Mother's education (SURVEY)
		label values base_edu*2p base_edu*4p survey_educ_2p_4p
		label values base_edu*2s survey_educ_2s
		
		foreach g in "2p" "4p" "2s" {
			foreach adult in "mother" "father" {
		...
		label values base_educ_cat_*_2p base_educ_cat_*_4p educ_cat
		label values base_educ_cat_*_2s educ_cat
		...
		*- Standardize ECE scores based on reference year (2007 for 2P and 2013 for the rest. Those are the years with 500 mean and 100 SD). This as opposed to yearly standardization.
		foreach g in "2p" "4p" "2s" {
			...
			replace base_score_math_`g' = (base_score_math_`g'-500)/100
			replace base_score_com_`g' = (base_score_com_`g'-500)/100
		}
